...
\renewcommand\lstlistingname{Code}
\begin{Java}[caption={\chk{The source code where the local variable table of its bytecode contains two entries with the same slot.}}, label={code:localVarTableDuplicatedSlot1SourceCode}]
...
\chk{The \code{try\{\}} block starts with pc 0 and ends with pc 40. The variable \code{url} is accessible from pc 13 onwards until pc 40 ([pc: 13, pc: 40]).}
...
We get the index of the \mr by using Javassist (in our example the index is 3), and with the index and pc information of the \mr, \chk{we can obtain the needed information from the local variable table shown in \bytecoderef{mrBytecodeExampleLocalVariableTable}.}
