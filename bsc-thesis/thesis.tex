...
\begin{quotation}
\noindent 
A previous study found out that NullPointerExceptions are very \ugh{serious} in Java projects. When a NullPointerExceptions occurs the developer is provided only with a stack trace to where the exception happened. \nm{NullPointerException is not ``serious", but is frequent and hard to debug. Please rephrase a bit this paragraph. Useful reference: \url{http://scg.unibe.ch/archive/papers/Osma15a.pdf}} This only gives insight into the effect of the fault but not into its cause. \ugh{So we have to ask the question when and why this reference was set to null.}\nm{Developer is the one presented only with stack trace, and he is the one WHO often asks a question of where this null is coming from.}

The aim of the project is to \del{be able }to provide \chg{the}{a} \chg{user}{developer} with an additional stack trace of where the value was actually set to null, next to the \chg{normal}{default} stack trace of a \ins{null pointer }exception. We attempt to achieve this goal by instrumenting java source code ideally with a minimal \ins{execution} overhead. 

\ugh{By tracking the null assignments the debugging after a NullPointerException will be simplified.} \nm{Make clear that tracking the assignments is done statically.}
\end{quotation}
\clearpage
...
Nowadays, certainly every programmer is confronted with NullPointerExceptions in \chg{big}{both big and small} Java Projects, whether it is for an enterprise or for private purposes. \nm{You should reference some paper stating the prevalence of NullPointerException. You can start the research from Haidar's paper already mentioned in the abstract.} \del{Not to mention even in small Java Projects they are also heavily present.}

So what are those NullPointerExceptions? \del{This thesis is going to attach importance to Java that is a concurrent, class-based, object-oriented programming language.}\nm{I get the idea that Java was not important before this thesis?! :)} \del{We chose Java because NullPointerExceptions are more serious in this language than in others, e.g. Smalltalk.}\nm{This is not true. NullPointerException is a problem commonly in object-oriented languages.} NullPointerException is a RuntimeException. In Java, an object reference can be assigned with a special null value. The exception is thrown when an application attempts to use an object reference that has the null value. \nm{Do not copy from online. \url{https://en.wikibooks.org/wiki/Java_Programming/Preventing_NullPointerException}} (There are multiple ways this exception can be thrown, like: Calling an instance method on the object referred by a null reference; Accessing or modifying an instance field of the object referred by a null reference and so on.) \nm{Why brackets?} \ugh{In Java Projects developers always have to deal with a huge amount of references which means avoiding these NullPointerExceptions is as good as impossible.}\nm{You can still have a big pile of objects, and none could be pointing to null.}

\ugh{On regular meetings among programmers they report what they have been doing and what they are planning to do for the next few weeks. But all too often it is stated that they are trying to fix bugs or have spent a lot of time fixing them. If there would be a way to minimize the time fixing exceptions and allow to work more efficiently, projects would progress much faster.}\nm{Rewrite: yes, developers deal with a lot of problems caused by null pointer exception, and you should find some paper to reference that. Also, not all bugs are caused by nulls. I would not mention weekly meetings.}

\nm{You should elaborate bit more on NullPointerException. Find some example in real code, not foo-bar example (JHotDraw can serve) for more insight.}

The main goal of the NullSpy \nm{What is NullSpy? You did not mention it till now.} application takes a step to that ideal vision. Anytime developers are facing a NullPointerException they \chg{don’t}{do not} have to spent time on debugging finding where and why a reference was set to null. With NullSpy the exact location of the null assignment is shown next to the ordinary stack trace the Java virtual machine produces.\nm{Got the meaning, but rephrase last two sentences into something like: Our intention with NullSpy is to present a developer the exact location of the null assignment next to the ordinary stack trace, in order to help him when searching for the place of dereference.}
...
\section{Javassist}
\label{sec:javassist}
Javassist or Java Programming Assistant\footnote{\url{http://jboss-javassist.github.io/javassist/}}\nm{You should reference paper related to Javassist}, a subproject of Jboss, is a class library which allows you to deal with Java bytecode. Since 1999 it is used as an engineering toolkit in a broad domain, and is still being extended by Shigeru Chiba. It enables developers to manipulate Java bytecode in a simplified way like defining a new class at runtime or modifying a class file when it is loaded by the JVM. All manipulations are performed at load-time through a provided class loader.
...
First a ClassPool object \del{is obtained} that controls bytecode modification with Javassist \ins{is obtained}. With the ClassPool a class file can be read on demand for constructing a CtClass object. The class CtClass (compile-time class) is just an abstract representation of a class file which means all manipulations are performed on the CtClass. With the method invocation \textit{get()} on ClassPool a reference to the class file \textit{test.Rectangle} is obtained\nm{File must have an extension, like .java or .class. So, do we obtain a reference to the file or to a \textit{class}?}. In this example the superclass of \textit{test.Rectangle} is just changed to \textit{test.Point}. \nm{Make picture with numbered lines, so that you can reference in each line what are you doing.} If the changes \nm{changes to what? on what?} are done, the method call \textit{writeFile()} on CtClass is necessary \ins{to call?} to make sure that the changes are reflected on the original class file.\footnote{Getting started with Javassist: \url{http://jboss-javassist.github.io/javassist/tutorial/tutorial.html}}\nm{Why footnote here? You should put it at the beginning of section.}
...
Figure \ref{fig:javassistModules} gives you an understanding/overview how the main part of bytecode manipulation with Javassist is built up. The ClassPool is nothing else than a container of multiple CtClasses. As described before CtClass is just the abstract representation of a class file \nm{file? or a class} on which modifications are done. Like typical classes, it can hold \del{several}\nm{several? not one?} compile-time fields, constants or methods. \ugh{While speaking about bytecode manipulation all the time, nothing but editing methods is manly meant.}\nm{I cannot parse this.} It is possible to insert additional source code at the beginning of the method body, at the end or at a specific line. \ugh{Next to these options even a \textit{\chg{catchBlock}{catch block}} can be added.}\nm{Is it next to these options? Where can you add the catch block? Isn't it among the options of adding code at the beginning, at the end or at specific point in the method code?}

\begin{figure}[H]
\centering
\includegraphics[width=0.9\linewidth]{insertionCodeExample}
\caption{Inserting code example\nm{Do you mention this figure somewhere?}}
  \label{fig:insertionCodeExample}
\end{figure}

\section{JAD}
\chg{JAva Decompiler}{Java Decompier (JAD)}\footnote{\url{https://sourceforge.net/projects/jadclipse/}} is a decompiler and a Eclipse plugin for the programming language Java. \chg{A short explanation what a decompiler is:}{A decompiler is} a computer program that takes an executable file as input, and attempts to create a high level, compatible source file \ugh{that does the same thing}. \chg{So it}{It} is used in software reverse engineering.\nm{Do you have a reference for this?}

\begin{figure}[H]
\centering
\includegraphics[width=0.6\linewidth]{bytecodeSourcecode}
\caption{Decompile example\nm{Do you mention this figure somewhere? Lines are blurry. Make clear what do you want to state with this.}}
  \label{fig:bytecodeSourcecode}
\end{figure}

\ugh{JAD is used in NullSpy since after running NullSpy on a project only the modificated bytecode\del{s} \chg{are}{is}\nm{bytecode is uncountable} available.} To simplify the check whether the modification by Javassist, e.g. inserting source code, has succeeded, a decompiler is needed. \nm{I am not sure this is the right place for JAD if you want to mention it. JAD was only used by *you* to be sure that the instrumentation is actually working, but it is not used in the final version of NullSpy? Or is it? If it is, then rewrite this section; it if it not and if you were using it just for the sake of debugging through NullSpy - you do explain that explicitely.}

\chapter{NullSpy}
As earlier explained in the \del{introduction} \charef{introduction} \nm{Please fix all the references} \del{(\ref{ch:introduction})}, this project is about providing the user with additional stack trace \chg{where}{with the information of} the origin of a \ugh{NullPointerException}\nm{It is not the origin of NullPointerException. It is the line of code where null is assigned to a variable.}\del{ is actually rooted}. \chg{Briefly worded}{In short}, it shows the developer the exact location of where a method receiver, which causes the NPE, was assigned to null. \nm{Well, it is not only the method receiver. Didn't you say before that NPE can be caused also by accessing or modifying fields of the null object.}

\del{This is the main chapter of the thesis.}\nm{You do not need to emphasise that this is the main chapter. The chapter title is revealing that.} \ugh{Here we would like to give you a short insight of how we managed to successfully implement the core of the project NullSpy. Next to how it is built up, we will also let you know what challenges we were encountering during the implementation and about the limitations we planned for future work (\ref{ch:conclusionsFutureWork}).} \nm{Please rephrase. Idea is fine, but you are talking directly to the reader: you should not ``let her know" but you are reporting about the implementation.}


\section{High level overview/Rough Scheme}
\label{sec:highLevelOverview}
The general approach of NullSpy is to statically analyze and add \del{additional} bytecode to a project. \nm{You should add a sentence or two to explain this more. Why statical analysis? Why adding bytecode?} \del{After reading the section Javassist (\ref{sec:javassist}) you should be more familiar with how bytecode manipulation with Javassist works.}

What NullSpy first does is loading the project you want it to be able to track the null assignment if a NullPointerException is thrown. By loading the project to NullSpy, the compiled class files of the project are addressed only, which means the project itself does not have to be imported to the programming environment, e.g. Eclipse. Simultaneously at load time each class file is modified with help of Javassist\chg{;}{.} In what way will be discussed in the following section \ugh{“Low level overview”.}

Once the project modification is done it is stored in a destination folder that the user has chosen before. This means after the changes there will be another version of the project which can \chg{do}{perform} additional \chg{stuff}{work} like tracking the null assignment. \nm{We don't want to use work 'stuff' in the thesis. :)} \chg{Because}{Since} only the class files are accessed previously, the result which is stored in the destination folder is as expected only the modified bytecode.

\ugh{The reason why we store the modified project in another directory than overwriting the existing one is because we would like to \del{only} involve Javassist library \ins{only} in the loading and editing part. This means the execution of the altered project does not include Javassist. This way the user does not have to download Javassist and integrate it to the project.}\nm{I don't understand. What would happen if the project is stored in the original destination? Please, elaborate.}

How do we check whether the instrumentation worked and the project really tracks the null assignment? The answer is wrapping the modified project into a jar file with which the modified project can be executed in the terminal or in Eclipse. \nm{Do we need to say this? Does end user really need to check whether the instrumentation worked or not? If he needs to check it, then it means that your code is not working... Also, how do you test if the instrumentation is working if you wrap it in jar file? What does it tell you?}
...
NullSpy only handles the unhandled NullPointerException, meaning the main method of a project is wrapped with a \chg{\textit{''catchBlock''}}{\textit{catch block}} \nm{Please use `` and " for quotes.} where the NPE is treated. The idea in this \chg{\textit{''catchBlock''}}{\textit{catch block}}  is to load the data about the assignments and about the method receiver to compare them with each other. \nm{From where do you get this data? How do you compare them? Why?} With the stacktrace\ins{,} the line number and the name of the Java class of where the NPE occured \nm{what?} can be figured out. We look them \nm{Who are them?} up in the method receiver information and get some hold points with which we look up in the assignment information. If there is a match we provide the user with an additional stacktrace next to the usual one. \ugh{Thanks to that link that points directly to the NPE root, debugging will be easier or it can even be skipped or avoided completely.}\nm{You can not state this.} To be able to provide this\ins{,} function data about the method receiver and variable assignment have to be collected initially. \nm{Please elaborate more in this paragraph. I get what do you want to say, but I was asking questions like somebody who has no idea of what you were talking about. Try to put yourself in a position of somebody who needs to understand the process.}

So we first load the project which should be modified and then go through all class files of it. A NullPointerException can only be thrown if a method call was performed on a method receiver which is null. \nm{Again, you are contradicting the story from beginning.} That means we have to gather information about the method receiver. \nm{What kind of information?} Next to this also information about the variable assignments has to be collected to know the exact location of the null assignment. The main idea behind NullSpy actually is to get information comparing those together when a NullPointerException occurs and if there is a hit during the comparison the  location of the null assignment can be obtained easily. \nm{You should add an example, and explain the approach through it. It will be much easier.}

The implementation behind the NullSpy concept will be explained more \chg{fully}{throughly}.
