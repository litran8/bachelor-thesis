...
However, we have encountered a persistent problem, namely getting the pc-interval of the \mr when the method invocation is split throughout multiple lines of code, thus when pc-interval covers multiple lines in source code.  
...
\chk{Let us now look at an example that contains the explained situation, where a method invocation is split into two lines. The example is shown in \coderef{4-1}. 
...
The second line only contains the parameters the method needs.}
...
Beforehand we have to say that there is no \mr in \coderef{4-1} because the \mr would contain a method invocation itself. NullSpy does not support \mrs which are return objects of method invocations. Enabling the coverage of such situations is a topic of future work. Other \mrs NullSpy does not support are elements of collections since a collection can hold other collections.
...
\chk{The line number table entries indicate the followings:
\begin{itemize}
...
\chk{To obtain the method invocation pc-interval we had to make further analyses. Only using the algorithm described in \subsecref{methodReceiver} we would only get <11,11> as the method invocation pc-interval. But the correct method invocation pc-interval in this example is <0,11>.}
...
\chk{The second example we look at is the example shown in \coderef{3-1}. In this example we see that the method invocation is situated in the second line unlike the invocation in the previous example.} This example is interesting because the line numbers 127 and 128 are stored in an alternated way in the line number table shown in \bytecoderef{3-3}. The line number table indicates followings:
...
\chk{First, the method invocation \code{getUndoActivity()} at line 128 is executed and then the execution order\lt{execution order ok? oder doch ein anderes wort?} jumps back to line 127 to perform the cast to \code{ChangeConnectionsHandle.UndoActivity}. After the cast, again we change to line 128 where the method \code{getOldConnector()} is invoked.}

\chk{With the further analyses we obtain <47,54> as the invocation pc-interval. 
...
\chk{The interesting part of third example (\coderef{2-1}) is the presence of multiple corresponding line numbers (line number table shown in \bytecoderef{2-3}), \ie lines 3 and 12, lines 4 and 11 and lines 5 and 10. The first line number indicates the starting point of an expression and the latter represents the ending point of the expression. Again, we examine the line number table:}

\begin{enumerate}
	\item line 3: starting point of the for-loop
	\item line 4: starting point of the method invocation \code{addItem(...)}
	\item line 5: \code{new ChangeAttributeCommand(...)} starting point\\or parameter loading onto the stack
	\item line 6-8: parameter loading onto the operand stack
	\item line 9-10: actual object creation \code{new ChangeAttributeCommand(...)}
	\item line 11: actual call to \code{addItem(...)}
...
By only using the algorithm of extracting the pc-interval of a \mr (described in \subsecref{methodReceiver}) we would get the wrong outermost pc-interval a the method invocation, when the invocation is split into multiple lines. To handle this mistake we implemented a supporting class called \code{MultipleLineManager}\footnote{\label{package2}In package \emph{ch.unibe.scg.nullSpy.instrumentator.controller.methodInvocation}.}. Its responsibility is to extract the outermost pc-interval of invocations in split line situations. 
\chk{For complexity reasons we omit the exmplanation how we implemented the class \code{MultipleLineManager}.}
...
One big problem we encountered was the insertion of the code if a variable assignment is the code before a closing curly bracket \code{\}}. 
We tried to insert code \chk{similar to the example as shown in \coderef{bytecodeModifier}} by specifying the exact line number where we want to enter in source code. 
...
%The last limitation we want to discuss here is that unfortunately NullSpy is not capable to track the root of \npes that are originated in the return value of a method call or in an element of a collection. 
%Why those situations are not supported in NullSpy is because of the impossibility way to store them, e.g. imagine a nested ArrayList or a HashMap and a never ending return value of method invocations. So we lack something tangible to compare with each other, get a hit and read the location out of the hit (\secref{futureWork}).
...
Let us have a look at the example\footnote{\url{http://ideone.com/rIFgT5}} shown in \coderef{demonstration}. \todo{Reference it}
...
\begin{enumerate}
\item The extraction of information about the \mrs because they are candidates that could cause a \npe. To achieve this, we developed an algorithm described in \subsecref{methodReceiver}. The algorithm finds the \mr, statically extracts the needed information about the \mr to create its unique identifier and stores the details.

\item Again collecting data, but this time about variable assignments: local variables and fields. For this purpose we statically analyze the bytecode and instrument the bytecode. We use Javassist to scan bytecode for instructions that represent variable assignments (regexs: \textit{``astore.*''} and \textit{``put.*''}). Finding such instructions we extract information about those assignments and pass the collected information to the code we insert after each variable assignment. On run time we check whether the assigned variable is assigned null. If this is the case the statically obtained details about that variable is stored.

\item NullSpy handles the uncaught \npe. That is why we wrap up the \code{main()} method with a \cblock. In this \cblock we extract details of where the \npe occurred from the \npe stack trace. With this data and the identifier for \mrs and variable assignments we perform several comparisons (\subsecref{processFlow}) to find the location of the null assignment which is the goal of NullSpy. 
\end{enumerate}
...
During the implementation we encountered many difficulties (\secref{challenges}). For most of the problems we came across with we managed to find a solution. Those unsolved ones could be proposed as goals of future work.
...
As mentioned in \secref{limitations}, if the culprit that caused the \npe is an element of a collection or the return object of a method invocation, NullSpy cannot track the \npe to the null assignment of the culprit variable. How to track the null assignment in the situation mentioned could be a future aim.
...
Last target for a possible future work is to transform NullSpy into an Eclipse compatible plug-in project. After integrated the NullSpy plug-in into Eclipse the null tracking can be started without any expenses on finding out how to use NullSpy on a project.
...
NullSpy is based on the instrumentation of code, that means the core work of NullSpy is to modify bytecode (with Javassist). Being able to modify bytecode had to learn how bytecode is constructed and to get familiar with Javassist. We went through a good Javassist tutorial\footnote{\url{https://github.com/jboss-javassist/javassist/wiki/Tutorial-1}} to learn how Javassist is used to instrument code on mostly on source-code level but also a bit on bytecode-level.

Due to the problem about how to extract information about \mrs, we spent a lot of time finding a way how this could be done. At the end we found the solution how this can be done. It was quite interesting to invent an algorithm that fulfills a more complex task.
...
%Luckily there is a very good tutorial that teaches one how to use it. This class library however does not cover everything we needed. Thanks to this we had to deal with the lack extensively and learned quite a lot about working on bytecode-level.
...
Coding beautifully so that the code does not smell was another challenge due to my lack of experience. Sometimes I tended to create god classes instead of following the single responsibility principle. Therefore, we had to refactor the whole project multiple times, which also took some time. 

%So I learndBest before starting to code is to clearly thinking through what is needed, how the structure should look like and what kind of responsibility each class of the project should take for gaining time for other things.
...
NullSpy is a program which helps Java developers to find the root of \npes by providing an additional link next to the common stack trace. The key idea behind NullSpy is to help developers to save time fixing bugs which are caused by \npes. This approach tries to provide the service mentioned by keeping the overhead to a minimum. To demonstrate how NullSpy works, this chapter serves as a small tutorial that takes JHotDraw as the testing project to show the feature of NullSpy.
