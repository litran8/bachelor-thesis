...
\Npe is a commonly occurring \rte in object-oriented languages. In most OOP languages there is a special \textit{null} value that is assigned to references in order to indicate that the reference does not refer to an object. A \npe is caused by invoking a method or accessing a field through a null-value reference, \ie a reference with the null value.
...
By calling \code{createIconkit()} at line 7 the field \code{fManager} (declared at line 3) is assigned \textbf{null}. The field is assigned \textbf{null} by performing the method \code{getIconkitManager()} at line 14, which just returns the \textbf{null} value at line 19.
