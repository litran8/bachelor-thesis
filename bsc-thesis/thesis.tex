...
\Npe is a commonly occurring \rte in object-oriented languages. \todo{Add some references} \brs{this todo has been here forever, do it or remove it}\lt{falls noch zeit übrig bleibt, werde ich noch mehr danach suchen. bis jetzt noch nichts gefunden.}In most OOP languages there is a special \textbf{null} value that is assigned to references in order to indicate that the reference does not refer to an object. A \npe is caused by invoking a method or accessing a field through a null value reference, \ie a reference with the null value.
...
Let us now look at a more complicated example of the local variable table {(\bytecoderef{localVarTableDuplicatedSlot1})\brs{<-- delete this reference, and move the source code above the byte code and bold the interesting parts}\t{because of the ``float'' I can't put the examples the way I want.}. Consider the source code example \coderef{localVarTableDuplicatedSlot1SourceCode} which contains a method \code{void loadDrawing(String filename)}. The bytecode of this example is shown in the \bytecoderef{localVarTableDuplicatedSlot1}.
This method consists of a \code{try-catch} block. 
We see that the variable \code{url} (declared at line 3 in \coderef{localVarTableDuplicatedSlot1SourceCode} and line 4 in \bytecoderef{localVarTableDuplicatedSlot1}) is only accessible within the \code{try\{\}} block. 
\chk{The \code{try\{\}} block starts with pc 0 and ends with pc 40. The variable \code{url} is build up from pc 0 on and is accessible fom pc 13 on.}
\del{which starts from pc \chg{0}{13}} \brs{please TRIPLE check all numbers}\lt{eigentlich fängt es mit 0 an, denn ab 0 wird die variable url erstellt und erst mit pc 13 richtig verfügbar. } and ends with pc 40.
