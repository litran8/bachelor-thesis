...
\renewcommand\figurename{Algorithm}
	\begin{boxedminipage}{\textwidth}
		\begin{enumerate}
\itemsep8pt
      \item Getting the pc-interval of method invocation
...
In case of the \textit{invokestatic} instruction we do not have a \mr \brs{why?}. That is why NullSpy treats it extraordinary by ignoring it completely or wraping it as a \ugh{possible \mr when it is actually a parameter of a method invocation}\brs{this does not make sense to me. if it's a parameter it could be a \mr?}. For all other invocation bytecodes we use our algorithm to get the \mr. 

\brs{the algorithm is very poorly explained, i still have no idea how you get from pc-interval to any useful information}

After getting the pc-interval of a \mr we gather data about it and store them in an external \textit{csv} file due to overhead minimization. The following list shows the information which is stored about the \mr.
