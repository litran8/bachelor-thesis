...
If the modified bytecode (\bytecoderef{modifiedBytecode}) were to be decompiled, its source code representation would be as shown in \coderef{modifiedCodeDecompiled}. At line 2 of this code, we can see the changes, \ie the inserted line of code. However, the actual result is the inserted bytecode represented by the pc-interval <0,5> in the modified bytecode (\bytecoderef{modifiedBytecode}).
...
%-------------------------------------------------------------------------------
\subsection{Process Flow - NullSpy Process Overview}
...
\chapter{Local Variable Assignments}
...
After each non-primitive variable assignment we insert bytecode that represents a method invocation. The invoked method is part of the class named \code{VariableTester}\footnote{\label{package1}In package \emph{ch.scg.nullSpy.runtimeSupporter}. This package is added to the modified project when the bytecode modification is done.}. 
