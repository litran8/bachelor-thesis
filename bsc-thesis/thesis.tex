...
\begin{quotation}
\noindent 
A previous study found out that \ugh{NullPointerExceptions}\brs{not sure if we can use NullPointerException as a word. We need to double check that} are the most frequently occurring \chg{furthermore}{and} \chg{very hard}{the most difficult} \brs{rock is hard, problems are difficult} to \del{be} debug\del{ged} exception\ins{s} in Java projects. \chg{It is}{They are}\brs{you started in plural (NullPointerExcemptionS) and switched to singular here (it). be consistent} hard to debug because the developer is only provided with a stack trace to where the exception \del{happened when a NullPointerException is} \ins{was} thrown. This only gives insight into the effect of the fault but not into its cause. \del{That is why the developer often asks where this null reference is coming from.}

The aim of the project is to provide a developer with an additional stack trace of where the \chg{value}{variable that caused the NullPointerException} was actually set to null\del{, next to the default stack trace of a NullPointerException}\brs{covered by the ``additioinal stack trace''}. We attempt to achieve this goal by instrumenting java source code \chg{ideally with}{striving for}  a minimal execution overhead. 

By tracking the null assignments \chg{by}{through} \chg{statically analyzing}{static analysis  and instrumentation of the} bytecode \del{and inserting additional code}\brs{implied in the ``instrumentation''} \ugh{the debugging after a NullPointerException will be simplified.}\brs{if you start the sentence with ``by doing X...'' you should end it with something like ``we can achieve Y'' ``developers can obtain Z'' \eg ``By genetically modifying cows we can make them produce chocolate milk.''} 
\end{quotation}
\clearpage
...
Nowadays, \ugh{certainly every programmer}\brs{except all the people that don't use Java} is confronted with NullPointerExceptions in both big and small Java projects, whether it is for an enterprise or for private purposes\brs{rephrase this to along the lines of ``Practically all Java developers are confronted with...''} . The seriousness of null dereferencing \chg{can be stated since it has been found out that in projects}{is apparent from the finding that } 35\% of the conditional statements \ins{in open source Java projects} are null checks which has a negative impact on performance \cite{Osma15a}\brs{Really? performance is what Haidar is complaining about in his paper? I honestly doubt that, i would bet it's readability of code.}.\latex{add a ~ instead of a space when using \cite{} to ensure it does not break the line there \eg performance~\cite{Osma15a}} It is also considered \del{as} the number one error Java programmers make\footnote{\url{http://www.javacoffeebreak.com/articles/toptenerrors.html}}.

\del{So what are those NullPointerExceptions?}\brs{too flowery. this is a technical paper, not literature, get to the point} NullPointerException is a commonly occurring RuntimeException in object-oriented languages. There is a special value \textit{null} \ugh{that is a pointer which does not point to a valid object, so it points to nowhere}\brs{the null value is not a pointer, it is assigned to pointers/references to indicate that the pointer/reference does not point to/reference an object}. The \ugh{main cause}\brs{and what are the other causes?} is a method invocation \ins{or an attempt to access a field} on a null object\brs{null objects are a different thing, they are concrete objects that represent a typed null value} \del{or an attempt to access a field}. This object reference can be a local variable, \chg{a}{an} instance field, an element of a collection, a return value of a function, \del{an object has not be initialized and so on}\brs{be mindful of this please! you just said that an object reference can be a a non-initialized object. object references and objects are two very distinct things, so please keep this in mind as you write to avoid this type of mistake}. In Java Projects developers always have to deal with a huge amount of references which means there is a high probability that there will be a NullPointerException.
...
Lets say \ins{that} in \coderef{npeExampleMethodReceiver} the variable at line 5 is assigned \del{with} the \ugh{special}\brs{are we gonna call it special every time?} null value \chg{thanks}{by} to the return value of the function \code{DNDHelper.processReceivedData()}. So the method receiver (\code{ff} at line 7) is null and when a method is called on this object a NullPointerException is thrown. Because the the NullPointerException occurred in the \code{try\{\}} block the \code{catch\{\}} block right after it handles the exception. The stack trace only takes the developer to line 7 where the exception is caused but not to the real culprit which is the assignment at line 5. This is a very simple example and the easiest way to produce a NullPointerException\brs{it's really not a simple example, we might consider finding a simpler one}. The root of the bug\brs{what bug? the NullPointerException is handled by the catch block} is located in the same method. \ugh{But in case a field access is performed it can be a lot more complicated. }\brs{WAT? a field access makes things more complicated? how? if you want to introduce your second example, just do it.}
...
With the \ugh{flow-sequence} \brs{flow-sequence? i guess you mean ``method invocations'' or something} \chg{of}{in} the method \code{open()} at line 10 the class instance \code{fManager} (line 4) is initialized \ins{by invoking the bla bla method} and \chg{at a later point it is accessed}{is accessed at a later point}\brs{your german is showing :) no need for the verb at the end}. Suppose the instance field \code{fManager} is set to null by the function \code{getIconkitManager()} at line 19\ins{ }when the method \code{createIconkit()} \ins{at line 12} is \chg{performed}{executed} \del{at line 12}. Later at line 14 the function \code{setTool()} is invoked which attempts an access to the field \code{fManager} that \ugh{points to nowhere}\brs{avoid this. just say it has a value of null}. That means a NullPointerException is thrown at line 25 \chg{and with the stack trace we only can track back the exception like}{with the stack trace shown in Figure X.Y} :
...
In \boxref{npe} \ins{we} can \chg{be seen}{see} that the stack trace does not track to the root of the NullPointerException. That means the Java developer has to debug to find the exception root\brs{not sure what an exception root is. either define it before \ie``we call the location where the variable was assigned the null value an exception root''. or say what you mean}
...
At this point we would like to introduce our project named \textit{\textbf{NullSpy}} which supports the developers in situations discussed previously\chg{,}{.} \chg{i}{I}n other words its main goal is to take a step to \ugh{that ideal vision}\brs{again, too flowery}. The intention behind NullSpy is to present the developers the exact location of the null assignment next to the ordinary stack trace, \del{in order to help him find the place of dereference}\brs{he already knows the place of dereference it's where the stack trace shows}.

In this thesis \ugh{it is explained}\brs{do not use passive, use royal we insted \ie we explain} how the goal mentioned above is achieved \del{step by step}, \del{by using a class library Javassist (Java Programming Assistant) which allows us to deal with Java bytecode.}\brs{not important, mention what else you do \ie and we also discuss the limitations and performance impact of the approach}

\chapter {Technical Background}
\label{ch:technicalBackground}

This chapter provides a short overview of technologies used in \del{this project that were essential for} the implementation of NullSpy.

\section{Javassist}
\label{sec:javassist}
\textit{Javassist} or \textit{Java Programming Assistant}\footnote{\url{http://jboss-javassist.github.io/javassist/}}\cite{Chib00}\cite{Chib03}, a subproject of Jboss, is a class library which \ugh{allows to}\brs{Oscar hates this, it is wrong to say ``allows to'' because you are missing a subject \ie ``allows me to'', ``allows John to''. use ``enables'' instead \ie ``enables manipulation of the bytecode''. please note it's not ``enables to'' because that has the same problem} \ugh{deal} with Java bytecode. Since 1999 it is used as an engineering toolkit in a broad domain, and is still being extended by Shigeru Chiba. It enables developers to manipulate Java bytecode in a simplified way\ins{.} \chg{like}{Examples of this manipulation include} defining a new class at runtime or modifying a class file when it is loaded by the JVM. All manipulations are performed at load-time through a provided class loader. \del{In case of more interest how Javassist works, we recommend to read the following tutorial: \url{http://jboss-javassist.github.io/javassist/tutorial/tutorial.html}}
\ins{A tutorial for using javasist\url{http://jboss-javassist.github.io/javassist/tutorial/tutorial.html} is available online and was used as a starting point for this work}.\brs{put the url in a footnote, it would not compile ins+footnote}
...
Unlike many other libraries Javassist offers two levels of API: \textbf{source}-level and \textbf{bytecode}-level (See \figref{bytecodeModificationLevels}). Using the source-level API, the user can edit a class file without any familiarity with the specifications of the Java bytecode. \chg{Only knowing}{Knowledge of} the Java language is enough \chg{because}{since} the API is designed only with the vocabulary of Java. On this level the programmer \del{just} has to write \chg{regular}{Java}\brs{is there irregular source?} source code and Javassist compiles it automatically. The bytecode level allows the user to modify classes \chg{directly in binary form}{my modifying the bytecode directly} \del{like other editors, e.g. ASM.}
...
cc.writeFile();
\end{Java}
\brs{a polymorphic call site? this code needs a better caption. }

\begin{enumerate}
	\item First a \textit{ClassPool} object that controls bytecode modification with Javassist is obtained. With the ClassPool\brs{be consistent with textit class names} a class file can be read on demand for constructing a CtClass object. By class file the binary file which has the extension \textit{.class} and represents either a Java class or an interface is meant. \brs{rewrite. super confusing. try using the phrase ``object representation of a file with the .class extension''. also, your german is showing with ``is meant'' at the end.}
	
	\item The class \textit{CtClass} (compile-time class) represents the class file\chg{, that means}{meaning that} all manipulations are performed on the CtClass \ins{object}. \chg{With the method invocation \code{get()} on ClassPool a reference to the CtClass object which represents the class \code{test.Rectangle} is obtained.}{We obtain a reference to the CtClass object representing the \code{test.Rectangle} class by invoking the \code{get()} method on the ClassPool instance.}
	
	\item In this example the superclass of \code{test.Rectangle} is \ugh{just}\brs{please stop saying just at random moments. If you wish to play down the significance of the action use ``simply''} changed to \code{test.Point}.
	\item \chg{If}{Once} the bytecode modification is done, the method call \code{writeFile()} on \ins{the instance of} CtClass is necessary to make sure that the changes are reflected on the original class file.
\end{enumerate}
...
\end{figure}
\brs{Captions are sentences and should end accordingly (. ! ?). please apply globaly}

\figref{javassistModules} gives an \del{understanding/}overview \ins{of} how the main part of bytecode manipulation with Javassist is built up. The ClassPool is \chg{nothing else than}{simply} a container of multiple CtClasses. As described before CtClass represents \del{those} class files on which modifications are done. Like typical classes, it can hold compile-time fields, constants or methods. Javassist is capable of adding or modifying classes, behaviors, fields, method invocations, local variables \chg{etc.}{\etc} But in our case we mainly address the manipulation of behaviors. It is possible to insert additional source code at the beginning of the method body\chg{,}{ or} at the end or at a specific line. 

\brs{not sure how the remainder here is relevant, please just give a nice example of some source code, how one would add code to it using javasist and the result}
Next to these options even a \textit{catch block} can be added. The difference between \code{insertAfter()} and \code{addCatch()} is insertAfter inlines some code just before every return instruction in the method body and addCatch adds a catch clause to the method which handles an exception thrown in the body. So the catch clause is always positioned at the end of the method body. In \figref{insertionCodeExample} below a small insertion example is demonstrated.
...
\section{JAD}
Java Decompier (JAD)\footnote{\url{https://sourceforge.net/projects/jadclipse/}} is a decompiler and a Eclipse plugin for the programming language Java. A decompiler is a \del{computer} program that takes an executable file as input, and attempts to create a high level, compatible source file. If the source file is compiled again, it will produce an executable program that behaves the same way as the original one. It is \ins{often} used in software reverse engineering.
...
\del{In NullSpy itself}, JAD is \del{not used} \ins{in no way a dependency of NullSpy} but it was a big help during the implementation phase. Since after running NullSpy on a project only the \chg{modificated}{modified}\brs{please use a spellcheck} bytecode is available. The use of JAD is for the sake of debugging \del{through} Nullspy; to check whether the modification by Javassist, e.g. inserting source code, has succeeded. Another way to check the result for correctness is looking at bytecode itself \chg{what}{but that} would have taken a lot \chg{of}{more} effort and time.

\chapter{NullSpy}
As \del{earlier} explained in \charef{introduction}, this project is about providing the user with \ins{an} additional stack trace \chg{with}{containing} the information \chg{of the origin or line of code of}{on} where \ins{the} null is assigned to \chg{a}{the} variable that caused the NullPointerException. In short, it shows the developer the exact location of where a method receiver\brs{I'm not sure method receiver is the right jargon for java. we need to check} or a field (in a field access case) was assigned \del{to} null \brs{assigning something to null means null=myVar}.

Here we \del{would like to } \brs{you are the author, don't tell the reader what you would like to do, he cares about what you did do} give a short \chg{insight}{overview}\brs{you cannot give insight} of how we \del{managed to successfully} implement\ins{ed} the core of \del{the project} NullSpy. Next to how it is built up, we will also address the challenges we \del{were} \chg{encountering}{encountered} during the implementation \chg{and}{as well as} the limitations\ins{ of the tool.} \del{ we planned for future work }(\charef{conclusionFutureWork}).

\section{High Level Overview/Rough Scheme}
\label{sec:highLevelOverview}
\brs{use just ``High level overview'' as the title}

The general approach of NullSpy is to statically analyze \ins{a project} and \chg{add}{instrument the} bytecode \del{to a project}. \ugh{We add bytecode to check on every variable for its value and extract information (name, line number etc.) about it if necessary.Another reason is to get information about method receivers} (\boxref{methodReceiverInfo}). \ugh{To know where exactly they are placed in source code we need to statically examine the code and insert the bytecode at the right place.}\brs{this part carries no information at all. I have no idea what you wanted to say here and why. are we not talking about high level overview here? \ie the architecture etc.?} 

\del{What} NullSpy first \del{does is }load\chg{ing}{s} the project that \chg{should be able to track the null assignment if a NullPointerException is thrown}{we wish to instrument}. \del{By} loading the project to NullSpy \ins{addressed only}, the compiled class files of the project \del{are addressed only}, which means the project \chg{itself}{source code} does not have to be imported \chg{to}{into} the programming environment, e.g. Eclipse. \ugh{Simultaneously at load time each class file is modified with help of Javassist. In what way will be discussed in the following }\secref{lowLevelOverview}\brs{please rephrase along the lines of ``as each class file is loaded the bytecode is modified as described in bla bla}.

Once the project modification is done it is stored in a destination folder that the user has chosen before\brs{How? it might be a good idea to discuss the command line arguments}. This means \ins{that} \del{after} the \chg{changes}{updated bytecode} \del{there} will be \ins{stored in }another \chg{version of the project which can perform additional work like tracking the null assignment}{location, keeping the original class files intact}. \del{Since only the class files are accessed previously, the result which is stored in the destination folder is as expected only the modified bytecode.}
...
\end{figure}
\brs{\url{http://www.merriam-webster.com/dictionary/instrumentator}. Also, you could break up the one block in to separate phases to make it more clear what happens.}

Of course the original project can be replaced by the modificated one too. But the developer has to think carefully about it because once replaced, the additional bytecode stays there and the execution time is slightly different than it was before.\brs{but one can always recompile to source to get the original class files when needed. this is an important point you should raise}
...
Unfortunately, Javassist does not provide the function to directly get the method receiver. We got a suggestion\brs{from whom?} to use AST (\todo{Abstract Syntax Tree footnote wiki}) to get it but we decided to not go deeper into this and implement our own algorithm.\brs{the whole suggestion thing is pointless then isn't it?}

The algorithm contains following steps \del{(abstract)}:
...
\caption{Local variable creation}
...
\begin{JVMIS}[xleftmargin=2em, framexleftmargin=1.5em, numbers=left, caption={Local variable table entries with same slot (line 5\&6)}, label={fig:localVarTableDuplicatedSlot}][numbers=left]
Local variable table:
...
An example where the instructions \textit{``putfield"} and \textit{``putstatic"} are used:
...
\tt{ch.scg.nullSpy.runtimeSupporter.VariableTesterdddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddd}

The added bytecode represents a static method of a class named \code{ch.scg.nullSpy.runtimeSupporter.VariableTester} which will be added to the modified project after class file modification is done. Depending on the kind of variable analyzed at the moment different bytecode is constructed, meaning different method with different parameter/variable data is added.
...
Once we have gone through the bytecode of a Java class, the modified class files are stored in a destination directory as mentioned in \secref{highLevelOverview}. Next to the instrumentation supplementary supporter classes are added to the project. The most important ones are \code{ch.unibe.scg.nullSpy.runtimeSupporter.VariableTester} which tests whether a variable is null or not and \code{ch.unibe.scg.nullSpy.runtimeSupporter.NullDisplayer} which matches data and prints the location of a null assignment when a NullPointerException is thrown.
...
This pseudocode will give a very rough idea how the invocation interval can be read out. The detailed implementation can be seen in the classes \code{ch.unibe.scg.nullSpy.instrumentator.controller.methodInvocation.MultipleLineManager} and \code{ch.unibe.scg.nullSpy.instrumentator.controller.methodInvocation.MethodInvocationAnalyzer}.
...
if (fName.startsWith("A")) {
	...
	ch.unibe.scg.nullSpy.runtimeSupporter.VariableTester.testLocalVar("", "", "", "", "", "", fontName, 0, 0, 0, 0, 0);
	fontName = new Calibri();
}
\end{Java}


That is why changed the way to insert the test method at bytecode-level. Like this we first have to build up the bytecode sequence and then enter it before a specific pc. Please take a look at the class \code{ch.unibe.scg.nullSpy.instrumentator.controller.ByteCodeAdapter} how the bytecode sequence is created.
