...
We extract this information with the help of Javassist, and below mentioned tables.
...
As already mentioned earlier, every time when encountering an assignment either \chg{of}{to} a local variable or a field, NullSpy inserts bytecode \chg{to check}{which check at run time} whether the variable is assigned null. If the variable is assigned null, NullSpy stores all the data about the variable. How we insert bytecode is described next.

%-------------------------------------------------------------------------------
\subsection{Bytecode Adaptation}
\label{bytecodeAdaptation}
Each time we encounter a variable assignment we first extract the \del{needed }data\ins{ about the variable} and then we \chg{ adapt the class file by adding extra }{add }bytecode right after the assignment bytecode. The \chg{function}{role} of the inserted bytecode is to check\ins{ at run time} whether the \chg{variable}{value being assigned to the variable} is null. If this is the case, \chg{the information collected before and which is passed as parameters to the checking method, is stored either into the \textit{``localVariableMap''} or into a \textit{``fieldMap''} which are HashMaps}{we store the previously explained information about this assignment}.
...
Depending on the category a field belongs to, different kind of information is stored about it. Number \chg{1}{one} in \listref{fieldKeywords} indicates fields of the current\ins{ly} analyzed class and number \chg{2}{two} can be static fields either of the current analyzed class or others classes. What Information we need about \chg{the}{these} field\ins{s} accessing \del{1\&2 }is listed in \listref{variableInfo1}.
