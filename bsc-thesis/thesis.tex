...
\begin{Java}[caption={\Npe example - \Mr.}, label={code:npeExampleMethodReceiver}]
...
\chapter{NullSpy}
As explained in \charef{introduction}, this project is about providing the user with an additional stack trace containing the information on where the null is assigned to the variable that caused the \npe. In short, it shows the developer the exact location of where a \mr\brs{I'm not sure method receiver is the right jargon (Ausdrucksweise) for java. we need to check} or a field (in a field access case) was assigned null.
...
analysis? Why adding bytecode?} \ugh{We add bytecode to check on every variable for its value and extract information (name, line number etc.) about it if necessary. Another reason is to get information about \mrs} (\boxref{methodReceiverInfo}). \ugh{To know where exactly they are placed in source code we need to statically examine the code and insert the bytecode at the right place.}\brs{this part carries no information at all. I have no idea what you wanted to say here and why. are we not talking about high level overview here? \ie the architecture etc.?} 
