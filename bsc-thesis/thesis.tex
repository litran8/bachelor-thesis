...
The \code{open(...)} method invokes the methods \code{createIconkit()} at line 7 and \code{setTool(...)} at line 9. By calling \code{createIconkit()} at line 7 the field \code{fManager} is assigned \textbf{null}. The field is assigned \textbf{null} by performing the method \code{getIconkitManager()} at line 14, which just returns the \textbf{null} value at line 19.
...
The second step is to collect information about the null assignments. After every non-primitive variable assignment we collect information about it and insert some bytecode right after the bytecode of the assignment. The bytecode we insert represents a static method of ours which is managed by the \textbf{run-time supporter} package. The collected information about the variable is passed as parameters to our static method. This method checks whether the variable is assigned null. If this is the case we store the information about the variable in a HashMap.
...
As we discuss the steps of the algorithm we will be showing the result of each step when applied to the example source code and bytecode shown in \coderef{methodReceiverAlgDemoCode} and \bytecoderef{methodReceiverAlgDemoBytecode}. In the example we are interested in the \mr \code{activePanel} of the \code{add(...)} method. That means we want to extract all information about \code{activePanel} listed in \listref{methodReceiverInfo}. The result of the algorithm will be the \mr pc-interval of the method \code{add(...)} (pc 155), \ie the pc-interval of \code{activePanel} (<144,144>). The algorithm contains following steps:

\nm{Why not use the example 2.1 and 2.2? Like this examples starts to pile up, and it becomes confusing.}
\lt{boris said that jumping in the pdf is confusing..}

\lt{boris-nevena differences}
...
We locate the pc at which the bytecode for source code line of the invocation begins. We call this the \textbf{start-pc}. This is done using the line number table mentioned in \secref{bytecode}. This table is omitted from our example for simplicity. The start-pc is 144.
...
Javassist offers the functionality to count the number of parameters of a method invocation by analyzing the method signature. The signature is of a type \textit{String} that contains the parameter and the return type of the method call. Our example method takes two  parameters.
...
Statically analyzing bytecode for \mr means looking for certain opcodes which matches the regex (Regular expression) \textit{``invoke.*''}. There are exactly five such bytecode instructions in Java: \textit{invokedynamic}, \textit{invokeinterface}, \textit{invokespecial}, \textit{invokestatic}, \textit{invokevirtual}.
...
When iterating through the bytecode and finding the instruction \textit{``astore.*''} we extract the slot from the opcode. For example we extract slot 2 from the opcode \textit{astore\_2} with pc 45. After extracting the slot (index: 2) we get the first local variable table entry which contains that slot (line 4 of \bytecoderef{localVarTableDuplicatedSlot}). If the pc of the local variable assignment is not included in the local variable lifespan of the entry (pc 45 $\notin$ [pc: 13, pc: 40], line 4), the next entry with the same slot (pc 45 $\in$ [pc: 44, pc: 94], line 7) will be examined. Checking the entries with the same slot goes on until both criteria slot and pc fits. Once those criteria are met we can be positive about having the right local variable table entry to extract the information we need.
...
With the line number table (shown in \bytecoderef{lineNrTable}) and the pc we can obtain the right line number. In the paragraph before we took a variable with pc 45 as an example, so the local variable assignment starts at line 64 according to the line number table. The line number is essential for constructing the link which will be added to the common exception stack trace.
