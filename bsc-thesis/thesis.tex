...
Let us now look at an example that contains the explained situation, where a method invocation is split into two lines. The example is shown in \coderef{4-1}. 
%The method invocation \code{registerAndLoadImage(...)} is formatted the way where it does not have enough space in one line. 
...
To find these kind of invocations, we need to look at the line number table.
%Only looking at bytecode is not enough, so we are looking at the line number attribute. 
Line number \textit{31} (\bytecoderef{4-3}) is listed twice, this indicates that the method invocation is split into multiple lines in source code. The former declares the starting point of the interval and the latter the end of it. \todo{Line 32 indicates the starting point of parameter loading.}
%How we get the pc-interval will be presented shortly. But before that we take a look at more examples.
...
We mentioned that NullSpy collects data about fields with help of the class \code{Javassist.expt.ExprEditor}. This class uses the method \code{loopBody(...)} to iterate through bytecode of a method and scans for specific opcodes, in our place we are only interested into the opcodes which matches the regex \textit{``put.*''}. One parameter passed to the method \code{loopBody(...)} is the \code{mehodInfo} object which contains the bytecode of the method we want to instrument.
...
One big problem we encountered was the insertion of the code if a variable assignment is the code before a closing bracket \code{\}}. 
We tried to insert code as shown in \figref{insertionCodeExample} by specifying the exact line number where we want to enter in source code. 
...
How we build the bytecode sequence inserted is explained in \todo{}.
