...
\renewcommand\lstlistingname{Code}
\begin{Java}[caption={The source code of the local variable table shown in \bytecoderef{localVarTableDuplicatedSlot1} belongs to.}, label={code:localVarTableDuplicatedSlot1SourceCode}]
...
\chk{The \code{try\{\}} block starts with pc 0 and ends with pc 40. The variable \code{url} is build up from pc 0 on and is accessible from pc 13 until pc 40.}
...
\end{Java}
\brs{indentation} \lt{verstehe nicht}
...
\chk{We use the term \textbf{enc-pc} to address the pc-s of these opcodes.}
...
Since bytecode lists method receiver and all the method parameters before the \textit{``invoke.*"} instruction, the pc of this \textit{``invoke.*''} opcode will represent the \textbf{end-pc} of a method invocation
\ugh{\brs{wtf is an end-pc? say that the invocation spreads over multiple pcs}}. In our example, the end-pc of the method invocation \code{add(...)} is 155.

\textbf{Finding the start-pc}\\ 
The \textbf{start-pc} is the pc at which begins the bytecode which represents the source code line, containing the invocation. Finding the start-pc is done by using the line number table of the method. We compare the \textbf{end-pc} with the \chk{entries of the line number table and take the last entry which has the smaller pc than the \textbf{end-pc}. The line number table of the example bytecode is shown in \bytecoderef{mrBytecodeExampleLineNumberTable}. (Since the entries in the line number table maps the beginning of the bytecode to the corresponding source code line, we call the pc in the last smaller entry the \textbf{start-pc})}. 
\ugh{\textbf{start-pc}-s for each line of code\brs{do all lines of code have a start-pc or just the ones with invocations?},}  The \textbf{start-pc} in our example is 144.
...
In our example, since the method takes two arguments, the actual \mr pc-interval is <144,144>. \brs{is this true if the method arguments are not all invocations? e.g. o.foo(bar, buzz());}\lt{verstehe nicht}
...
We get the index of the \mr by using Javassist (in our example the index is 3), and with the index and pc information of the \mr, \chk{we can obtain the needed information of the local variable table shown in \bytecoderef{mrBytecodeExampleLocalVariableTable}.}
...
There are exactly five such bytecode instructions in Java: \textit{invokedynamic}, \textit{invokeinterface}, \textit{invokespecial}, \textit{invokestatic}, \textit{invokevirtual}, \textit{invokedynamic}. \brs{woah, what? context switch out of nowhere! how does this relate to Appendix C and method receivers?}\lt{wohin dann? oben, wo wir über reges matching schreiben?}

In case of the simple stand-alone \textit{invokestatic} instruction we do have a \mr but it can be ignored because static method invocations are called on classes which can never be null. NullSpy ignores these instructions. But if the static method call is a parameter of another method call, NullSpy still treats it as a \mr candidate. As for the other \textit{invoke} instructions, they are treated normally.\brs{including invokedynamic which is not used by java?}\lt{inovke dynamic nie in jhotdraw angetroffen -> was jetzt?}
