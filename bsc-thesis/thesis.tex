...
\Npe is a commonly occurring \rte in object-oriented languages. \todo{Add some references} \brs{this todo has been here forever, do it or remove it}In most OOP languages there is a special \textit{null} value that is assigned to references in order to indicate that the reference does not refer to an object. A \npe is caused by invoking a method or accessing a field through a null-value reference, \ie a reference with the null value.

Previous research has found that 35\% of conditional checks in Java projects are null checks~\cite{Osma16a}. This reduces the readability of source code and has a negative impact on performance~\cite{Osma16a}\brs{two identical references for two consecutive sentences is lame, just remove the first one}. It is also considered the number one error Java programmers make\footnote{\url{http://www.webcitation.org/6lNPzbIyy}}.
...
The \code{open(...)} method invokes the methods \code{createIconkit()} at line 7 and \code{setTool(...)} at line 9. By calling \code{createIconkit()} at line 7 the field \code{fManager} is assigned \textbf{null}\brs{null was code a while ago now it's bold. be consistent}. The field is assigned \textbf{null} by performing the method \code{getIconkitManager()} at line 14, which just returns the \textbf{null} value at line 19.
...
In \stacktraceref{stackTrace} we can see that the stack trace points only to the line 24, where \npe occured, and it does not track to the location where the variable is assigned the null value. We call this location the root of an exception. That means the Java developer has to debug \chg{his/her}{their} way to the exception root.
...
\del{We extract this information with the help of Javassist, and below mentioned tables.}\brs{what we do is irrelevant in technical background}
%In \secref{lowLevelOverview} we will see how this information is used.
...
The line number table\footnote{In the jargon of Javasist, this is also called the line number attribute.} maps source code line of the method with the pc which indicates the beginning of the bytecode to the corresponding source code line. The line number table in \bytecoderef{helloWorld} is represented between the lines 9 and 12: source code line 5 starts with the pc 0 (\chg{represented by the}{as shown on} line 10), source code line 6 starts with the pc 5 (in line 11) \etc This mapping does not apply to cases where a statement is separated into multiple lines. \todo{Should we move next explanation to the appendix?}\del{In }\subsecref{methodReceiverDifficulties} \del{we explain further details about the line number table in a split Java statement case.}\brs{irrelevant, just say that this should be noted as it will have great impact on our implementation}

If we look back, to the \stacktraceref{stackTrace}, we see that the stack trace provides us with \chg{the numbers of lines of code, which are part of the call-chain that led to \npe}{a part of the call-chain that led to the \npe. This includes the line numbers in the source code where the invocations happened.} The information the line number table holds is used to recover the information about the actual point in the code where the null was assigned to a reference. 
%In \secref{lowLevelOverview} we will discuss how this is done.

\renewcommand\lstlistingname{Bytecode}
\begin{JVMIS}[numbers=left, caption={Local variable table entries with same slot 2 (line 4 and 7).}, label={bytecode:localVarTableDuplicatedSlot1}, breaklines=true]
Local variable table:
...
Let us now look at a more complicated example of the local variable table {(\bytecoderef{localVarTableDuplicatedSlot1})\brs{<-- delete this reference, and move the source code above the byte code and bold the interesting parts}. \del{Local variables do have a limited lifespan. }\chg{Let us have a look at}{Consider} the source code example \coderef{localVarTableDuplicatedSlot1SourceCode} which contains a method \code{void loadDrawing(String filename)}. The bytecode of this example is shown in the \bytecoderef{localVarTableDuplicatedSlot1}.
This method consists of a \code{try-catch} block. We see that the variable \code{url} (declared at line 3 in \coderef{localVarTableDuplicatedSlot1SourceCode} and line 4 in \bytecoderef{localVarTableDuplicatedSlot1}) is only accessible within the \code{try\{\}} block which starts from pc \chg{0}{13} \brs{please TRIPLE check all numbers} and ends with pc 40.
This local variable is stored at the index 2 in the local variable table in \bytecoderef{localVarTableDuplicatedSlot1}, since the first two indexes occupy \textit{this} reference and the method parameter. \del{Variable \code{url} is created at the pc 13 and is only accessible until pc 40 (excluded)}\brs{already said in last paragraph}. 
...
Looking at line 7 in \bytecoderef{localVarTableDuplicatedSlot1} we see that \code{e} also has the slot number 2. This is why we can find local variable tables that contain multiple entries with the same local variable slot.\brs{this paragraph is HUGE! Split the URL talk in to one paragraph, and the reuse part in another}

The indexes of the the local variable table are important because we use them to get the right local variable we are interested in. \del{How we make use of the indexes to find the aimed variable is explained in }\subsecref{variable}. \brs{the first sentence should be where you first mention the indexes, the second is irrelevant}

%===============================================================================
\section{Javassist}
\label{sec:javassist}

\textit{Javassist} or \textit{Java Programming Assistant}\footnote{\url{http://jboss-javassist.github.io/javassist/}}~\cite{Chib00}~\cite{Chib03}, a subproject of \chg{Jboss}{JBoss}, is a library which enables manipulation of the Java bytecode. Since 1999 it is used as an engineering toolkit in a broad domain, and is still being extended by Shigeru Chiba. It enables developers to manipulate Java bytecode in a simplified way. Examples of this manipulation include defining a new class at runtime or modifying a class file when it is loaded by the JVM. All manipulations are performed at load-time through a provided class loader. A tutorial for using Javassist\footnote{\url{http://jboss-javassist.github.io/javassist/tutorial/tutorial.html}} is available online and was used as a starting point for this work.
...
\del{At this point, }let us look at the small example shown in \coderef{javassistExample}\footnote{Example taken from Javassist tutorial.} of how the bytecode manipulation with javasist works. We go through the example line by line.
...
\begin{enumerate}
	...
	\item In this example the only bytecode modification done is changing the superclass of \code{test.Rectangle} to \code{test.Point}. \ugh{This does not serve any purpose and is only illustrative.}\brs{Illustration is a purpose, so say ``This change serves only for illustration''}
...
\end{Java}
\brs{I feel the captions are a bit uninformative. For example: ``Bytecode example.'' has no context whatsoever, especially since we had a ton of bytecode examples}
%===============================================================================
\section{JAD}
\label{sec:jad}
...
As explained in \charef{introduction}, this project is about providing the user with an additional \ugh{link }next to the common stack trace containing the location of a \textit{null} assignment to the variable that caused the \npe.
...
In this section we explain how to get information about the \mr needed to construct a unique identifier. \Mrs in the example \coderef{methodReceiverAlgDemoCode} \brs{examples in appendix? nope!} are the variables \code{fToolButton} at line 7, \code{fToolButton.myIcon} at line 8 and \code{activePanel} at line 11.
...
To do this, NullSpy looks for the opcodes which matches the regex \textit{``invoke.*''}. Each of these opcodes represents one method invocation. 
...
Since bytecode lists method receiver and all the method parameters before the \textit{``invoke.*"} instruction, the pc of this \textit{``invoke.*''} opcode will represent the \textbf{end-pc} of a method invocation\brs{wtf is an end-pc? say that the invocation spreads over multiple pcs}. In our example, the end-pc of the method invocation \code{add(...)} is 155.

\textbf{Finding the start-pc}\\ 
The \textbf{start-pc} is the pc at which begins the bytecode which represents the source code line, containing the invocation. Finding the start-pc is done by using the line number table of the method. We compare the \textbf{end-pc} with the \textbf{start-pc}-s for each line of code\brs{do all lines of code have a start-pc or just the ones with invocations?}, and take the last one which is smaller than the \textbf{end-pc}. The \textbf{start-pc} in our example is 144. \todo{Add tables to the bytecode C.1}
...
In our example, since the method takes two arguments, the actual \mr pc-interval is <144,144>. \brs{is this true if the method arguments are not all invocations? e.g. o.foo(bar, buzz());}
...
We get the index of the \mr by using Javassist (in our example the index is 3), and with the index and pc information of the \mr, we can 
\todo{explain till the end}

To create a unique identifier for a method receiver we need \ugh{the information like}\brs{``like'' implies similar, do we need this data or similar data?}:
\begin{itemize}
...
There are exactly five such bytecode instructions in Java: \textit{invokedynamic}, \textit{invokeinterface}, \textit{invokespecial}, \textit{invokestatic}, \textit{invokevirtual}. \brs{woah, what? context switch out of nowhere! how does this relate to appendix C and method receivers?}

In case of the simple stand-alone \textit{invokestatic} instruction we do have a \mr but it can be ignored because static method invocations are called on classes which can never be null. NullSpy ignores these instructions. But if the static method call is a parameter of another method call, NullSpy still treats it as a \mr candidate. As for the other \textit{invoke} instructions, they are treated normally.\brs{including invokedynamic which is not used by java?}
...
\begin{JVMIS}[numbers=left, caption={Local variable table entries with same slot 2 (line 4\&7\brs{do not use \&}).}, label={bytecode:localVarTableDuplicatedSlot}, breaklines=true]
Local variable table:
 [pc: 0, pc: 95] local: this index: 0 type: org.jhotdraw.samples.javadraw.JavaDrawViewer
 [pc: 0, pc: 95] local: filename index: 1 type: java.lang.String
...
\todo{add assignment bytecode}
Let us assume that we have a local variable assignment where its \textit{``astore\_2''} opcode is located at pc 45 (\bytecoderef{localVarTableDuplicatedSlot})\brs{Clearly not in this bytecode}. In this example we can extract slot 2 from the opcode \textit{astore\_2}. After extracting the slot (index: 2) we iterate through the local variable table and find the first entry which contains that slot (line 4 in \bytecoderef{localVarTableDuplicatedSlot}). If the pc of the assignment is not included in the local variable lifespan of the entry (pc 45 $\notin$ [pc: 13, pc: 40], line 4), the next entry with the same slot (pc 45 $\in$ [pc: 44, pc: 94], line 7) will be examined. Checking the entries with the same slot goes on until both criteria slot and pc fits. Once those criteria are met we can be positive about having the right local variable table entry to extract the information needed to create a unique identifier for the variable. We now extract from local variable table the information we need to create a unique identifier for a variable like\brs{same as before about ``like''}:
\begin{itemize}
...
\label{subsec:field}
Although Javassist does not support the access to\ins{ information about} local variable\ins{s} it provides a way to access\ins{information about} fields. Javassist allows us \nm{\url{http://scg.unibe.ch/wiki/howtos/commonwritingerrors/neverSayAllowsToAgain}}\brs{remove this link if you have seen it} to modify an expression in a method body by using the class \code{Javassist.expt.ExprEditor}. 
...
It scans the bytecode \chg{on}{for} instructions like \textit{``putfield''} for instance\brs{instance what?} or \textit{``putstatic''} for static fields, and allows us to directly get the information we need to create a unique identifier for the field, like\brs{again}:
\begin{itemize}
	\item source code line number of the assignment
...
As already mentioned earlier, every time when encountering an assignment either to a local variable or a field, NullSpy inserts bytecode which checks at run time whether the variable is assigned null. If the variable is assigned null, NullSpy stores all the data about the variable. \del{How we insert bytecode is described next.}

%-------------------------------------------------------------------------------
\subsection{Bytecode Adaptation}
\label{bytecodeAdaptation}
Each time we encounter a variable assignment we first extract the data about the variable and then we add\ins{ our own} bytecode right after the assignment bytecode. The \del{role of the }inserted bytecode \del{is to }check\ins{s} at run time whether the value being assigned to the variable is null. If this is the case, we store the previously explained information about this assignment.

The explanation of the way we do it can be found in the \chg{a}{A}ppendix \ref{cha:appInstrumentation}.
