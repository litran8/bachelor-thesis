...
On the left-side of these instructions we see the \textbf{program counter} (pc) which is a processor register that contains the address (location) of the instruction being executed at the current time. It is also called an \textbf{instruction pointer}. We use these program counters, or pc-s also as an index in the bytecode. The instructions from pc 0 to pc 4 (lines 2-4) in \bytecoderef{helloWorld} represents the source code at line 5 in \coderef{helloWorld}. Therefore we use the term \textbf{pc-interval} to describe a set of instructions that represents a Java expression. This term will play an important role further in the thesis. We use the syntax \textbf{``<,>''} to represent an pc-interval, \eg just mentioned pc-interval <0,4> represents the source code at line 5 in \coderef{helloWorld}. The first number indicates where the pc-interval starts and the latter one where it ends, both inclusive.

If the opcode represents the read from a variable, on its right side there is the information about the name and type of the variable, \eg line 3. 
In case of an invocation opcode, it shows the behavior/method name, the parameters it takes and the return type, \eg line 7. In \secref{lowLevelOverview} we will see how this information is used.
...
The line number table\footnote{In the jargon of Javasist, this is also called the line number attribute.} maps the pc to the source code line of the method. The line number table in \bytecoderef{helloWorld} is represented between the lines 9 and 12: source code line 5 starts with the pc 0 (represented by the line 10), source code line 6 starts with the pc 5 (in line 11) \etc. This mapping does not apply to cases where a statement is separated into multiple lines. In \subsecref{methodReceiverDifficulties} we explain further details about the line number table in a split Java statement case.

If we look back, to the \stacktraceref{stackTrace}, we can see the numbers at the end of each line. They indicate the lines of code, which are the part of the call-chain that led to \npe. The information the line number table holds is used to recover the information about the actual point in the code where the null was assigned to a reference. In \secref{lowLevelOverview} we will discuss how this is done.
...
Local variable table:
 ...
 [pc: 13, pc: 40] local: url index: (@\textbf{2}@) type: java.net.URL
...
In our example, \textit{this} reference has the lexical scope of the whole method, thus pc starts at 0 and ends with 13. \textit{This} reference is the first one in the table, hence it is indexed with 0, and its type is class \code{HelloWorld}.

The indexes of the the local variable table are important because we use them to get the right local variable we are interested in. How we make use of the indexes to find the aim variable is explained in \subsecref{variable}.

Let us now look at a more complicated example of the local variable table (\bytecoderef{localVarTableDuplicatedSlot1}). Local variables do have a limited lifespan. Let us have a look at the source code example \coderef{localVarTableDuplicatedSlot1SourceCode} which contains a method \code{void loadDrawing(String filename)}. The bytecode of this example is shown in the \bytecoderef{localVarTableDuplicatedSlot1}.
This method consists of a \code{try-catch} block. We see that the variable \code{url} (declared at line 3 in \coderef{localVarTableDuplicatedSlot1SourceCode} and line 4 in \bytecoderef{localVarTableDuplicatedSlot1}) is only accessible within the \code{try\{\}} block which starts from pc 0 and ends with pc 40.
