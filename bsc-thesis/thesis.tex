...
\Npe is a commonly occurring \rte in object-oriented languages. \todo{Add some references} In most OOP languages there is a special \textit{null} value that is assigned to references in order to indicate that the reference does not refer to an object. A \npe is caused by invoking a method or accessing a field through a null-value reference, \ie a reference with the null value.

\del{Nowadays, most, if not all Java developers are at some point confronted with \npes in both big and small Java projects.} Previous research has found that 35\% of conditional checks in Java projects are null checks~\cite{Osma16a}. This reduces the readability of source code and has a negative impact on performance~\cite{Osma16a}. It is also considered the number one error Java programmers make\footnote{\url{http://www.webcitation.org/6lNPzbIyy}}.
...
\nm{@BS: Do you agree to skip the first example, as it is very easy and just go straight to the second one?}

A more complicated way a \npe can be triggered is when it involves variables with larger scopes such as class members. \coderef{npeExampleFieldAccess}\textsuperscript{\ref{codeTaken1}} shows such a situation. \ins{This code snippet defines the class named \code{DrawApplication} which has the field \code{fManager} (declared at line 3). This class contains four relevant methods: \code{open(...)}, \code{createIconkit()}, \code{getIconkitManager()} and \code{setTool(...)}.}

The \code{open()} method \chg{calls}{invokes} the methods \code{createIconkit()} at line 7 and \code{setTool()} at line 9.
...
By calling \code{createIconkit()} at line 7 the field \code{fManager} is assigned \textbf{null}. The field is assigned \textbf{null} by performing the method \code{getIconkitManager()} at line 14, which just returns the \textbf{null} value at line 19.

Afterwards, when the method \code{open()} calls \code{setTool()}, the attempt to call a method\ins{ \code{getComponent()}} on\ins{ field} \code{fManager} at line 24 causes the \npe. The resulting \npe stack trace is shown in \boxref{stackTrace}:
...
In \boxref{stackTrace} we can see that the stack trace\ins{ points only to the line 24, where \npe occured, and it} does not track to the location where the variable is assigned the null value. We call this location the root of an exception. That means the Java developer has to debug his/her way to the exception root.
...
We are going to look only at the bytecode of method \code{say()} which is represented in between the lines 25-39 in \bytecoderef{helloWorld}. That means we will only focus on the second part of the example \bytecoderef{helloWorld}, \ie from line 23 onwards. The green words in \bytecoderef{helloWorld} are the string representations of Java bytecode instructions. We will also use the term \textbf{opcode} when referring to individual instructions. Multiple opcode instructions are referred to as bytecode.

On the left-side of these instructions we see the \textbf{program counter} (pc) which is a processor register that contains the address (location) of the instruction being executed at the current time. It is also called an instruction pointer. We use these program counters, or pc-s also as an index in the bytecode. The instructions from pc 0 to pc 4 (lines 26-28) in \bytecoderef{helloWorld} represents the source code at line 5 in \coderef{helloWorld}. Therefore we use the term \textbf{pc-interval} to describe a set of instructions that represents a Java expression. This term will play an important role further in the thesis. We use the syntax \textbf{``<,>''} to represent the pc-interval. For the example just mentioned the pc-interval <0,4> represents the source code at line 5 in \coderef{helloWorld}. The first number indicates where the pc-interval starts and the latter one where it ends, both inclusive.
...
Each method holds a \textbf{line number table} and a \textbf{local variable table} which are listed underneath the instructions of the method. We first look at the line number table\footnote{In the jargon of Javasist, this is also called the line number attribute.} of method \code{say()} which is represented from line 33 until line 36. This line number table maps the pc to the source code line of the method \code{say()} ( line 25 to 39). In situations where the Java statement is not divided in multiple line, the line number table indicates the starting pc of each line of code. This does not apply to cases where an expression is separated into multiple line. In \subsecref{methodReceiverDifficulties} we explain further details about the line number table in a split Java statement case.

If we look back, to the \boxref{stackTrace}, we can see the numbers at the end of each line. They indicate the lines of code, which are the part of the call-chain that led to \npe. The information the line number table holds is used to recover the information about the actual point in the code where the null was assigned to a reference. In \secref{lowLevelOverview} we will discuss how this is done.
...
As mentioned before the bytecode of each method also holds a local variable table (\eg \bytecoderef{localVarTableDuplicatedSlot1}) which is an array of the information about local variables. This table has the information about the \textit{this} reference if the method is an instance method or a constructor, about method parameters and method local variables, respectively. Each line of this table represents one variable: it has

\begin{enumerate}
	\item the pc-s which represent the lexical scope of the variable,
...
Local variables do have a limited lifespan, \ie the pc-s in between the square brackets of the local variable table represents the lifespan of the variable. The local variable \code{url} at line 4 in \bytecoderef{localVarTableDuplicatedSlot1} (corresponding source: \coderef{localVarTableDuplicatedSlot1SourceCode}) is not created until pc 13 and is only accessible until pc 40 (excluded). As soon as the lifespan of a local variable ends, the slot can be reused by the next instantiated local variable. See the local variable table example shown in \bytecoderef{localVarTableDuplicatedSlot1} and the corresponding source code \coderef{localVarTableDuplicatedSlot1SourceCode}. As explained earlier, line 4 in \bytecoderef{localVarTableDuplicatedSlot1} holds the information about the local variable \code{url} which can be found at line 3 in \coderef{localVarTableDuplicatedSlot1SourceCode}. We see in the source code example \coderef{localVarTableDuplicatedSlot1SourceCode} that the variable \code{url} is only accessible within the \code{try\{\}} block which starts from pc 0 and ends with pc 40. If we look further than pc 40 at bytecode-level, slot 2 is free again. The freed slot can be  immediately used by the next variable, which is the variable \code{e} in our example. Looking at line 7 in \bytecoderef{localVarTableDuplicatedSlot1} we see that \code{e} also has the slot number 2. This is why we can find local variable tables that contain multiple entries with the same local variable slot.
...
We provide the option to choose the destination in case the developer does not want to overwrite the original project with the modified one. This means that the updated bytecode can be stored in another location, keeping the original class files intact. Of course the original project can also be replaced by the modified one. But the developer has to think carefully about it because once replaced, the additional bytecode stays there and the execution time is slightly different than it was before (explanation can be found in the \secref{execTimeDiff}). To sweep away the additional bytecode, the developer has to recompile the source code to get the original class files when needed.
...
Loading is done by traversing through all the subfolders of the folder whose local path was given as a parameter, and extracting all the \textit{.class} files. NullSpy modifies the class file directly when it is loaded. As soon as the modification is done, the class file is stored at the destination folder \ie the folder whose path is the second parameter of NullSpy. The structure of the source original folder is preserved also at the destination folder.
...
After class loading, we first extract data from the class bytecode, then instrument it and at the end we add the run-time supporter module to the project.
\nm{This paragraph is not exactly true. You do not add run-time support every time you load and instrument a class. You add it just once, at the end, right? That said, every class actually goes through two and three phases?} \lt{better?}
...
From the stack trace of a \npe we can obtain three kind of information, namely the class name, method name and the line number of where the \npe happened. But the aim of NullSpy is to reveal the location of the null assignment to the variable which caused the \npe. To achieve this goal we need to take information from the bytecode about \textbf{\mrs} and \textbf{null assignments}. Why we need to extract data about \mrs and null assignment is explained right away. 

As soon as a \npe is thrown, we sequentially compare data to get the right location of the null assignment. The comparison-sequence looks as follows:

\begin{enumerate}
	\item \npe data \textbf{$\|$} \mrs data $\rightarrow$ result\_1
	\item result\_1 \textbf{$\|$} null assignment data $\rightarrow$ \textbf{result\_2}
\end{enumerate}

Result\_2 contains the data needed to create the link to the null assignment.

The first step in the modification part is to gather information about the \mrs. We use the term \textbf{\mr} to refer to variables on which methods are invoked, or whose fields are accessed. What kind of information we store about the \mrs is listed in \subsecref{methodReceiver}. The reason why we need to store information about the \mrs is because they are candidates that could trigger a \npe. To find the location where the \mr is assigned null we form a unique identifier out of the gathered information. With the identifier we are able to find the null assignment entry that holds all information about where the variable is assigned null. The most important information is the line number of where the variable is assigned null. With that number we can build the link which takes the user directly to the location where the null assignment took place.

%\nm{At the beginning of this paragraph, first explain what is the idea: 
%why do you store anything about variables which are used as receivers for methods. 
%Then define the term ``method receiver", and precisely say what kind of information do you store, and why. 
%When you define the term ``method receiver", say by which information is a method receiver uniquely identified: class name, method name etc.}
%
%\lt{delete this paragraph?}
%The first step in the modification part is to gather information about the \mrs. We use the term \textbf{\mr} to refer to variables on which methods are invoked, or whose fields are accessed. The same data as obtained from the \npe stack trace (class name, method name, line number) and other values \eg the variable name, variable type \etc are stored in an external csv file. \nm{where?}
%\nm{Do not give example of what are you storing; say precisely what kind of information are you storing.}
%\nm{Overall, you use too much sentences like ``we gather data", or ``we gather information". Always be precise: what kind of information.}

The second step is to collect information about the null assignments. After every non-primitive variable assignment we collect information about it and insert some bytecode right after the bytecode of the assignment. The bytecode we insert represents a static method of ours which is managed by the \textit{run time supporter} package. The collected information about the variable is passed as parameters to our static method. This method checks whether the variable is assigned null. If this is the case we store the information about the variable in a HashMap.

%If a variable is assigned an object, we gather data about that variable from the bytecode.\\
%\nm{Here for example, you can actually leave the sentence like this ``we gather data" since you already explained in the previous paragraph exactly what kind of information do you store.} 
%
%The most important information obtained is the \textit{line number} of the null assignment.\\
%\nm{Because of the previous sentence, I asked ``an object or null"?}
%
%Right after the assignment and the data collection \nm{You are here confusing two things: right after the assignment (you refer to place in the code?) and right after data collection (you here refer to the time - when?) }NullSpy inserts the bytecode that represents a method call which is managed by the \textit{run time supporter} package. This method takes those extracted information as parameters and checks whether the variable is assigned null. \nm{Ok, I suppose it was actually ``an object" at the beginning of the paragraph?} \nm{When does it check whether the variable is null? At static time or at run time?} \chg{If it is assigned null}{if this is the case,} the information about the variable is stored.

At the end of the bytecode modification part we wrap the \code{main()} method into a \textit{try-catch-block}. In the \textit{catch-block} those three information mentioned before about the place in code where \npe happened (class name, method name, line number) are extracted from the \npe stack trace. This information is passed on to a static method of the run time supporter package. The method compares the parameters with the stored information about the \mrs. \nm{It feels like something is missing. NullSpy compares class name, method name and LOC of the place where \npe happened with method receivers? Don't you need also the information about the variables used in that line of code so that you can compare this with the stored data about method receivers?} If there is a match regarding the comparison, NullSpy creates a variable identification with the information about the matched \mr. With this id another comparison is performed. This time NullSpy compares the id with the information stored about the null assignment. The result will be the stored entry about the null assignment of the variable that caused the \npe. From the entry we extract the line number of the assignment and create the additional link that reveals the location of the null assignment. This link of course is then added to the common \npe stack trace.

Finally we add the \textit{run time supporter} module to the modified project.

The way how we extract and store all those data are discussed in the following sections. \nm{These last three paragraphs are really confusing...}

%-------------------------------------------------------------------------------
\subsection{\MR Data Collection}
\label{subsec:methodReceiver}
As mentioned in \subsecref{processFlow}, a big part of NullSpy is extracting information from bytecode. One of the item we are interested to get information about is the \mr \nm{My suggestion is to make this name ``method receiver" in italic for example, just to distinguish it as a term every time you mention it. What do you think?}. A small synthetic example (\coderef{methodReceiverExample}) makes is easier to understand. \Mrs in \coderef{methodReceiverExample} are the variables \code{var} at line 9, \code{field} at line 10 and \code{b.foo} at line 11.
...
We use the library Javassist to extract data from bytecode. Unfortunately, Javassist does not provide the functionality to directly get information we need about the \mr\, thus we need to do it manually. This is why we implemented our own algorithm to get the \mrs. The exact information we need is listed in \listref{methodReceiverInfo} and it is stored in an external \textit{csv} file due to overhead minimization. We need the exact \textit{pc-interval} of the \mr to extract the right data from the bytecode. \nm{What is right data? Listed in 3.4?}
...
As we discuss the steps of the algorithm we will be showing the result of each step when applied to the example source code and bytecode shown in \coderef{methodReceiverAlgDemoCode} and \bytecoderef{methodReceiverAlgDemoBytecode}. \nm{State want information you want to extract. At the end of the next paragraph, you say ``In our example, the end-pc is 155, since we are interested in the receiver of the \code{add} method." Say here: ``We are interested in the information blah, blah, blah...}The algorithm contains following steps:

\nm{Why not use the example 2.1 and 2.2? Like this examples starts to pile up, and it becomes confusing.}

\lt{boris vs nevena}
\textbf{Finding the invocation bytecode}\nm{Why not put this paragraph before previous paragraph?}\\
While iterating through the bytecode, NullSpy looks for the opcodes which matches the regex \textit{``invoke.*''}. Each of these opcodes represents one method invocation\ins{.} \chg{for which we are trying to find the \mr.}{For each method invocation we want to find the method receiver.} We call the pc of the invocation the \textbf{end-pc}. In our example, the end-pc is 155, since we are interested in the receiver of the \code{add} method.

\textbf{Finding the start-pc}\\ 
\nm{Isn't this start-pc also part of the invocation bytecode? Shouldn't the previous paragraph be something like ``Finding the end-pc", and together with this paragraph be a subparagraphs of the ``Finding the invocation bytecode"?}
We locate the pc at which the bytecode for source code line of the invocation begins. We call this the \textbf{start-pc}. This is done using the line number table mentioned in \secref{bytecode}. \chg{This table is omitted from our example for simplicity and the start-pc is 144.}{This table is omitted from our example for simplicity. The start-pc is 144.} \nm{Do not put two completely independent information into one sentence. It doesn't make sense.}
...
Javassist offers the functionality to count the number of parameters\ins{ of} a method invocation \del{has }by analyzing the method signature. The signature is\ins{ of a type} a \textit{String} \nm{It is not the class String itself!} that contains the parameter and the return type of the method call. Our example method takes \chg{2}{two} \nm{Always spell out small numbers.} parameters.

\textbf{Extracting candidate \mr pc-intervals}\\
The outermost interval <144,155> could contain other invocations, which could have \mrs of their own. This is visible in our example, where the \code{getDesktop} invocation (pc 146) is embedded in our outermost interval. For this reason, we explored all possible bytecode combinations of \mrs and developed a system for identifying such embedded call sights. More about this system can be found in the appendix (\ref{cha:bytecodeCombinations}). This system allows us to split the outermost pc-interval into candidate \mr pc-intervals. In our example, the outermost pc-interval is divided into \{<144,144>,<145,149>,<152,152>\}. \nm{Explain what each of these intervals represent.}

\textbf{Finding the actual \mr}\\
Since the candidate pc-interval\del{s} end\ins{s} with the arguments of the method, and we know that the method expects $N$ arguments, we can ignore the last $N$ candidate pc-intervals. The one before them is the actual \mr pc-interval. In other words, the $N$-th to last candidate is the actual \mr pc-interval. In our example, since the method takes two arguments, the actual \mr pc-interval is <144,144>.
...
\begin{itemize}
    ...
    \item variableID (distinguishes whether the variable is a field or a local variable)
		...
		\item startPc
		\item storePc
		\item afterPc (right before this pc additional bytecode is inserted)
		\item className
		\item behavior
...
\label{subsec:field}
Although Javassist does not support the access to local variable it provides a way for fields. Javassist allows to modify an expression in a method body with the class \code{Javassist.expt.ExprEditor}. In our case we only want to extract some information about the fields instead of any modification, nevertheless this class can be used appropriately. What it does is to scan the bytecodes on instructions like \textit{``putfield''} for instance or \textit{``putstatic''} for static fields.
...
Depending on the category a field belongs to, different kind of information is stored about it. Number 1 in \listref{fieldKeywords} indicates fields of the current analyzed class and number 2 can be static fields either of the current analyzed class or others classes. What Information we need about the field accessing 1\&2 is listed in \listref{variableInfo1}.
...
\item memberObject:
\begin{itemize}
\item memberObjectName
\item memberObjectType
\item memberObjectDeclaringClassName
\item isMemberObjectStatic
\item memberObjectOpcode (to distinguish if it is a local variable or a field)
\end{itemize}
   \end{itemize}
	\end{boxedminipage}
	\caption{Field information (II).}
...
As already mentioned before, every time after finding an assignment either of a local variable or a field, NullSpy enters bytecode to check whether the variable is assigned null. If the variable is assigned null, NullSpy stores all the data about it in a HashMap. How we insert bytecode is described next.
...
Now we will go through the code snippet shown in \coderef{bytecodeCreation} and explain it. The method header of the behavior \code{VariableTester.testDirectField()} shown in \coderef{checkFieldMethodHeader} takes seven strings, one boolean, one object and four integers as parameter. The snippet \coderef{bytecodeCreation} reproduces those parameter. 

From line 4 to 17 the instruction \textit{``addLdc''} is used to add strings to the bytecode. For adding a boolean we used the integers \textit{``1''} and \textit{``0''} to represent the boolean value \textit{true} respectively \textit{false}. Code-lines 20 to 25 adds integers to the bytecode sequence. 

The Object, in our case a field, is added by the method \code{addAload()} at line 27 and the method \code{addGetfield()} at line 28. In our synthetic example shown in \coderef{fieldExample} we see that how a field is assigned something at line 11. The bytecode of this example is \textit{``aload\_0'', \textbar\dots\textbar, ``putfield''}. Even if we don't write the keyword \code{this} in front of the field \code{b}, the bytecode will be the same. The example was about the field assignment, but if we only want to load a field the bytecode would look like \textit{``aload\_0'', ``getfield''}. Back to our bytecode creation example \coderef{bytecodeCreation}, lines 27 and 28 are exactly adding those instructions to the bytecode. We take the variable itself as parameter to evaluate its value in the check-method.

Important integers like the line number and pcs are added to the bytecode sequence from line 31 to 24. Finally all classes that the check-method takes as parameter are prepared at lines 36 to 39 and the instruction \textit{``invokestatic''} is added to the bytecode.

With the classes \textit{CodeAttribute} and \textit{CodeIterator} from the Javassist API we can enter the bytecode sequence we created into existing ones. CodeAttribute represents the bytecode of a behavior and with CodeIterator we can iterate through that bytecode. So we first move the CodeIterator to the position (pc) in the bytecode where we want to enter our bytecode with \code{codeIterator.move(pc)} and than enter our sequence with \code{iter.insert(ourByteCode)}.

There were again some challenges we had to get over, like getting the right data after an instrumentation and entering the additional code in the right position of the bytecode sequence. In (\subsecref{bytecodeAdaptationDifficulties}) we discuss about our problems we had.

Once we have gone through the bytecode of a Java class, the modified class files are stored in the destination directory as mentioned in \secref{highLevelOverview}. After the instrumentation we add supplementary supporter classes to the project. The most important ones are \code{VariableTester}\textsuperscript{\ref{package1}} which tests whether a variable is null or not and \code{NullDisplayer}\textsuperscript{\ref{package1}} which matches data and prints the location of a null assignment when a \npe is thrown.
...
\renewcommand\figurename{Box}
	...
	\label{box:demonstrationStackTrace}
\end{figure}
