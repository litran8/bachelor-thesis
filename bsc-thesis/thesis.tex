...
\chk{To obtain the method invocation pc-interval we had to make further analyses. By using the algorithm described in \subsecref{methodReceiver} we would get <11,11> as the method invocation pc-interval, which is incorrect. The correct method invocation pc-interval in this example is <0,11>.} \todo{Be sure the results are correct.}
...
\chk{With the default analyses we would obtain \todo{<?,?>} as the invocation pc-interval, whereas the correct one is <47,54>. 
...
By using the algorithm of extracting the pc-interval of a \mr (described in \subsecref{methodReceiver}) we would get the wrong outermost pc-interval of the method invocation. To handle this mistake we implemented a supporting class called \code{MultipleLineManager}\footnote{\label{package2}In package \emph{ch.unibe.scg.nullSpy.instrumentator.controller.methodInvocation}.}. Its responsibility is to extract the outermost pc-interval of invocations in split line situations. 
...
In situations where an element of a collection causes a \npe, we cannot refer to that element. We cannot identify that element if the collection it belongs to is again stored in another collection \etc. Hence, we cannot identify collection elements.
...
\chk{After the implementation of NullSpy we validated it with help of the project JHotDraw. First we measured the time how long JHotDraw takes to execute all its tests without touching the bytecode. After that we instrumented the project and again measured the execution time. The result is a performance overhead of approximately 5\%. So NullSpy could help the developer finding the null assignment without a big impact on the performance.}
...
NullSpy is a program which helps Java developers to find the root of \npes by providing an additional link next to the common stack trace. The key idea behind NullSpy is to help developers to save time fixing bugs which are caused by \npes. This approach tries to provide the service mentioned by keeping the overhead to a minimum. To demonstrate how NullSpy works, this chapter serves as a small tutorial that takes a small example source code as the testing project to show the feature of NullSpy.
...
\begin{enumerate}
	\item Download example source code from \url{http://ideone.com/rIFgT5}
	\item Make sure to get the class file of that Java class, \eg import that class into eclipse and execute it
	\item Checkout the project NullSpy from \ursl{https://github.com/litran8/nullpointer-javassist}
	\tiem 
\end{enumerate}}
...
%Extracting candidate method receiver pc-intervals
%The outermost interval <144,155> could contain other invocations, which could
%have method receivers of their own. This is visible in our example, where the
%getDesktop() invocation (pc 146) is embedded in our outermost interval. For
%this reason, we explored all possible bytecode combinations of instructions that can
%represent method receivers and developed a system for identifying such embedded call
%sights. More about this system can be found in the Appendix (B).
...
We developed a system that checks whether the combinations of specific opcodes are valid. The combinations for identifying the candidate \mr pc-intervals are listed below:  

		\begin{itemize}
		\item non-static field
		\begin{itemize}
			\item \checkmark aload\_0 (aload\_0 = ref for private method)
			\item \checkmark aload\_0.getfield
			\item \checkmark aload\_0.getfield.getfield... (public fields)
			%\item $\times$ aload\_0.this (aload\_0 = ref, this... = arg)
			%\item $\times$ aload\_0.getfield.getstatic (!exist)
			%\item $\times$ aload\_0.getstatic (aload\_0 = ref, getstatic = arg)
			%\item $\times$ aload\_0.aload (aload\_0 = ref, aload = arg)
			%\item $\times$ aload\_0.new (aload\_0 = ref, new = arg)
		\end{itemize}

		\item static field
		\begin{itemize}
			\item \checkmark getstatic.getfield
			\item \checkmark getstatic.getfield.getfield... (public fields)
			%\item $\times$ getstatic (ignore, because getinvoke)
			%\item $\times$ getstatic.aload
			%\item $\times$ getstatic.aload\_0
			%\item $\times$ getstatic.new
			%\item $\times$ getstatic.getstatic (!exist)
		\end{itemize}
		
		%\item object creation
		%\begin{itemize} 							
			%\item \checkmark new...(put.*/astore.*)
		%\end{itemize}
		
		\item local variable
		\begin{itemize}
			\item \checkmark aload
			\item \checkmark aload.getfield 							
			\item \checkmark aload.getfield.getfield... (public fields)
			%\item $\times$ aload.getstatic (aload = ref, getstatic = arg)
			%\item $\times$ aload.aload\_0 (aload = ref, this.getfield etc = args
			%\item $\times$ aload.aload (aload = ref, aload = arg)
			%\item $\times$ aload.new (aload = ref, new = arg)
		\end{itemize}
   \end{itemize}

To understand how this list is used we go through an example to illustrate it:

When we iterate through the outermost invocation pc-interval and encounter the opcode \textit{``aload.*''} we check what comes after this opcode. If \textit{getfield} is the following opcode, we know that the bytecode represents a field access of a local variable or of the current analyzed class depending on which local variable slot the opcode \textit{``aload.*''} refers to. Again, we check the next opcode after \textit{getfield} \etc 
...
If the following opcode after the first \textit{``aload.*} is another \textit{``aload.*} we can be sure that they are two separate \mr candidate because two local variables does not have any relationship with each other.

For each outermost invocation pc-interval we use this template to split it into candidate \mr pc-interval. 
...
We explained that we extract \mr information to create its unique identifier for the several comparisons which have to be done to find out the location of the null assignment of the \mr. In the subsection we describe the algorithm how it extracts the \mr pc-interval, we only gave a hint of what kind of information we need to create the unique identifier. The complete list of what data we store about the \mr is listed next:
...
To find the location of the null assignment NullSpy has to perform several comparison with the identifier of \mrs, local variables and fields. To create those identifiers we have to collect information about those variables mentioned. We only listed part of the information needed to create a identifier of a local variable. Here is the full list:
...
We discussed about the reason why we have to extract information about the fields. Bytecode has only two instructions that indicate field assignments. Even though we distinguish more than two \textit{``different''} kinds of fields. 

With the term \textit{``different''} we mean that there are different kinds of instruction sets that represents a field assignment. What we mean with \textit{``different''} kinds of fields is demonstrated in the next listing. But beforehand let us see at the placeholder \textbar\dots\textbar which is used in the listing. This placeholder indicates a set of instructions which represents the value that is assigned to a field. In other words, it represents the object which is loaded onto the operand stack for the assignment. Now let us see at the \textit{``different''} kinds of fields:
...
\begin{enumerate}
	...
	represents an assignment to a static field\\
	...
	We use the term \textbf{member field} to describe a field declared in another class than the class being analyzed.\\
	...
	\eg Class.field...memberField =  \textbar\dots\textbar;\\
	\eg Class.memberField...memberField =  \textbar\dots\textbar;\\
...
We talked about the collection of needed information about fields to create its unique identifier for comparison reasons, we only listed few data which we actually extract from bytecode about the assigned field. If a field of the current analyzed class is assigned a value, we extract following information about it:   
...
The aim of NullSpy is to find the location of the null assignment of the variable which caused the \npe. We achieve this goal by instrumenting bytecode with help of the bytecode-level API of Javassist.

After each non-primitive variable assignment we insert bytecode into the original one. 
The bytecode we add represents a method of a class named \code{VariableTester}\footnote{\label{package1}In package \emph{ch.scg.nullSpy.runtimeSupporter}. This package is added to the modified project when the bytecode modification is done.}. 
Depending on what variable (local variable, field) we analyze for the moment, different bytecode is constructed. With this we mean that the bytecode we create can represent different method invocations which accepts different arguments/variable data.

We mentioned that Javassist provides the way to modify bytecode either with the source- or bytecode-level API. Due to a limitation of Javassist we modify the class files at bytecode-level by constructing a bytecode sequence and include it into the file. How we build this sequence is shown in \coderef{bytecodeCreation}.
...
The code snippet (\coderef{bytecodeCreation}) creates the method invocation that tests a field assignment whether it is assigned null. The header of that method is presented in \coderef{checkFieldMethodHeader}.
...
Now we go through the code snippet and explain it. This method takes seven strings, one boolean, one object and four integers as arguments. The code snippet reproduces these arguments. 
