...
\begin{quotation}
\noindent
A previous study found that \npes are the most frequently occurring exceptions in Java projects. Also, it is difficult to debug because a developer is only provided with a stack trace to the line of code where the exception was thrown. This only gives insight into the effect of the fault but not into its cause.
...
By tracking the null assignments through static analysis and bytecode instrumentation we can achieve a more efficient debugging process after an occurrence of a \npe.
\end{quotation}
\clearpage
...
\Npes are a commonly occurring \rte in object-oriented programming (OOP) languages. In most OOP languages there is a special \textbf{null} value that is assigned to references in order to indicate that the reference does not refer to an object. A \npe is caused by invoking a method or accessing a field through a null value reference, \ie a reference with the null value.
...
Assume that in the example shown in \coderef{npeExampleMethodReceiver}\footnote{The code snippet is a modified version of code taken from JHotDraw project: \url{http://www.jhotdraw.org/}} the variable \code{ff} at line 5 is assigned the null value as the result of assigning the return value of the method \code{DNDHelper.processReceivedData(...)} to the variable. This means that the same variable \code{ff} is null at line 7 and when a method is invoked on this object a \npe is thrown. Because the \npe is raised in the \tblock the following \cblock will handle the exception by printing the stack trace. The stack trace takes the developer to line 7 where the exception was thrown but not to the real culprit which is the assignment at line 5.
...
\ins{The execution stack holds basically no information about the cause of the exception}~\cite{Libl05a}.
...
\ins{There are other tools which can help developers debug and find the root cause of a bug. Most notably the Back-in-time debuggers~\cite{Lien08b}. The main differences between NullSpy and Back-in-time debuggers are:}

\begin{enumerate}
	\item \ins{NullSpy targets a particular problem, namely the \npe, while back-in-time debugging is more general.}
	\item \ins{Back-in-time debugging might require the developer to step back many times before finding the root of the exception while NullSpy finds it directly.}
\end{enumerate}

\ins{Another approach to which could help find the root cause of a \npe is the Object Flow Analysis \ie tracking the object flow}~\cite{Lien08d}\ins{. The main difference here to NullSpy is that NullSpy tracks the null assignments instead of the object flow.}
...
\begin{JVMIS}[numbers=left, caption={Bytecode from method \code{say()} in \coderef{helloWorld}.}, label={bytecode:helloWorld}, breaklines=true][H]
public void say();
	 0  aload_0 [this]
...
The green words in \bytecoderef{helloWorld} are the string representations of Java bytecode instructions. We will also use the term \textit{\textbf{opcode}} when referring to individual instructions. Multiple opcode instructions are referred to as \textit{\textbf{bytecode}}.

On the left-side of these instructions we see the \textit{\textbf{program counter}} (pc) which is a processor register that contains the address (location) of the instruction being executed at the current time. It is also called an \textbf{instruction pointer}.

The instructions from pc 0 to pc 4 (lines 2-4 in \bytecoderef{helloWorld}) represent the source code at line 5 in \coderef{helloWorld}. Therefore, we use the term \textit{\textbf{pc-interval}} to describe a set of instructions that represents a single Java expression. This term will play an important role in later sections. We use the syntax \textbf{``<x,y>''} to represent an pc-interval, \eg the mentioned pc-interval <0,4> represents the source code at line 5 in \coderef{helloWorld}. The first number indicates where the pc-interval starts, thus we name it \textit{\textbf{start-pc}} and the latter one where it ends, hence the name \textit{\textbf{end-pc}}, both inclusive. 
...
Each method bytecode representation holds a \textit{\textbf{line number table}} and a \textit{\textbf{local variable table}} which are listed underneath the instructions of the method (lines 9-15 in \bytecoderef{helloWorld}). 

The line number table maps the source code line of the method to the pc that indicates the beginning of the bytecode to the corresponding source code line. The line number table in \bytecoderef{helloWorld} is represented between the lines 9 and 12: source code line 5 starts with the pc 0 (as shown on line 10), source code line 6 starts with the pc 5 (in line 11) \etc This mapping does not apply to cases where a statement is separated into multiple lines.
...
\renewcommand\lstlistingname{Code}
\begin{Java}[caption={The source code for which the local variable table of its bytecode contains two entries , \code{url} and \code{e}, that share the same slot.}, label={code:localVarTableDuplicatedSlot1SourceCode}]
...
\begin{figure}[H]
\centering
\includegraphics[width=0.8\linewidth]{uml}
\caption{Javassist modules.}
  \label{fig:javassistModules}
\end{figure}

\figref{javassistModules} gives an overview of how the main part of bytecode manipulation with Javassist is built up. The \textit{ClassPool} is simply a container of multiple \textit{CtClasses}. As described before \textit{CtClass} represents a class file on which modifications are done. Like typical classes, it can hold compile-time fields, constants or methods. Javassist is capable of adding or modifying classes, behaviors/methods, fields, method invocations, local variables \etc But in our case we mainly address the manipulation of behaviors. It is possible to insert additional source code at the beginning of a method body, at the end or at a specific line. 
...
\ins{As a JAD illustration we decompile the bytecode shown in }\bytecoderef{initialBytecode} \ins{of the source code example }\coderef{initialCode}\ins{ and indicate the differences between the decompiled and the original version of the source code. The result of the decompilation is shown in} \coderef{decompiledBytecode}.

\renewcommand\lstlistingname{Code}
\begin{Java}[caption={The result of the decompiled bytecode shown in \bytecoderef{initialBytecode}.}, label={code:decompiledBytecode}]
...
\ins{JAD imported the class \code{java.io.PrintStream} at line 1 and generated a default constructor at line 5. As already mentioned, we see that the decompiled version of the class \code{HelloWorld} behaves the same way as the original version, even the decompiled source code does not look identically as the original one.}
...
\ins{The general approach of NullSpy is to statically analyze a project in which a \npe occurs and instrument the bytecode. NullSpy statically analyzes the project to extract data from bytecode about variables, \ie the name of the variable, the source code line number where it is located \etc We instrument the code to get the value stored in those variables at run time.} 

NullSpy is a console application that takes two arguments. The first argument to NullSpy is the local path to the folder containing the compiled class files of the original project and the second one is the path to the folder in which the user wants to store the modified class files.
...
\ins{The module \textit{``Bytecode data extractor''} extracts data from bytecode. 
The second module \textit{``Instrumentation''} inserts bytecode into the original code. 
Lastly, the module \textit{``Run-time supporter''} adds the package that implements the functionality to which the previously inserted bytecode refers to.}
...
%-------------------------------------------------------------------------------
\subsection{NullSpy Process Overview}
...
From the stack trace of a \npe, we can obtain three kinds of information: the class name, method name and the line number of where the \npe happened. The aim of NullSpy is to reveal the location of the null assignment to the variable which caused the \npe. To achieve this goal we need to extract information from the bytecode about \textbf{\mrs} and \textbf{null assignments}.

The first step in the modification part is to statically gather information about the \mrs \ins{ and the variable assignments}. We use the term \textit{\textbf{\mr}} to refer to variables on which methods are invoked, or whose fields are accessed.
...
\ins{For every non-primitive variable assignment we also perform static analysis to collect information about the variable. The information again is needed to create its unique identifier.}
...
public class DrawApplication {
	...
	protected void open(final DrawingView newDrawingView) {
		...
		(@\textbf{activePanel}@).add((Component)getDesktop(),BorderLayout.CENTER);	// local variable as a target variable
		...
	}
}
\end{Java}
...
\renewcommand\lstlistingname{Bytecode}
\begin{JVMIS}[numbers=left, caption={Bytecode of line 11 from \coderef{methodReceiverAlgDemoCode}.}, label={bytecode:methodReceiverAlgDemoBytecode}, breaklines=true]
...
\subsubsection{Finding the Invocation Opcode/End-pc}
...
To get data about \mrs we iterate through the bytecode looking for the instructions that represent a method invocation.
To do this, NullSpy looks for the opcodes that match the regex \textit{``invoke.*''}. Each of these opcodes represents one method invocation. There are four invocation bytecode instructions in Java bytecode: \textit{invokeinterface}, \textit{invokespecial}, \textit{invokestatic}, \textit{invokevirtual}.

For each invocation we want to find the \mr. 
In our example the bytecode instruction that matches the invocation of the method \code{add(...)} is at line 6 in \bytecoderef{methodReceiverAlgDemoBytecode}.
Since the bytecode lists the \mr and all the method parameters before the \textit{``invoke.*"} instruction, the pc of this \textit{``invoke.*''} opcode will represent the \textbf{end-pc} of the method invocation pc-interval. In our example, the end-pc of the method invocation \code{add(...)} is 155.

\subsubsection{Finding the Start-pc}
The \textbf{start-pc} of the method invocation pc-interval is the pc at which the bytecode representing the source code line containing the invocation begins. The start-pc is found by using the line number table of the method. We compare the \textbf{end-pc} with the entries of the line number table and take the last entry that has a smaller pc than the \textbf{end-pc}. The line number table of the example bytecode is shown in \bytecoderef{mrBytecodeExampleLineNumberTable}. 
...
We define the \textit{\textbf{outermost pc-interval}} as the interval starting with the \textbf{start-pc} and ending with the \textbf{end-pc}. We know that the \mr is located in this interval. In our example, this is <144,155>.

\subsubsection{Extracting Candidate \MR Pc-intervals}
The outermost pc-interval <144,155> could contain other invocations, which could have \mrs of their own. This is visible in our example, where the \code{getDesktop()} invocation (pc 146) is embedded in our outermost pc-interval. For this reason, we explored all possible bytecode combinations of instructions that can represent \mrs and developed a system for identifying such embedded call sites. More about this system can be found in the Appendix (\ref{sec:bytecodeCombinations}). This system allows us to split the outermost pc-interval into candidate \mr pc-intervals. In our example, the outermost pc-interval is divided into \{<144,144>,<145,149>,<152,152>\}. The candidate pc-intervals represent:
...
\subsubsection{Finding the Actual \MR}
...
If the \mr is actually the return value from another method invocation, it is not apparent where exactly the \mr is situated in bytecode. More about the challenges we encountered \ins{and why we cannot directly assume that the first \mr candidate is the one we are actually looking for }can be found in \subsecref{methodReceiverDifficulties}.
...
\begin{itemize}
	\item source code line number of the \mr
...
The exact information we need can be found in the Appendix \ref{sec:appReceivers}.
...
To find the assignments in the bytecode we search for the instructions matching regexes \textit{``astore.*''} and \textit{``put.*''}. The former refers to local variable assignments and the latter to field assignments. 
...
Every time we encounter the opcode \textit{``astore.*''}  (which represents an assignment to a local variable) we get the index of the variable and the pc of the assignment instruction. A local variable has a scope. As we have already seen in \secref{bytecode}, multiple entries can have the same local variable slot (one variable takes the index of the other whose lifespan has expired).

\begin{JVMIS}[numbers=left, caption={Local variable table entries with same slot 2 (lines 4 and 7).}, label={bytecode:localVarTableDuplicatedSlot}, breaklines=true]
Local variable table:
 [pc: 0, pc: 95] local: this index: 0 type: org.jhotdraw.samples.javadraw.JavaDrawViewer
 [pc: 0, pc: 95] local: filename index: 1 type: java.lang.String
 (@\textbf{[pc: 13, pc: 40] local: url index: 2 type: java.net.URL}@)
...
Let us assume that we have a local variable assignment where its \textit{``astore\_2''} opcode is located at pc 45 (\bytecoderef{localVarAssignment}). In this example we can extract slot 2 from the opcode \textit{astore\_2}. After extracting the slot (index: 2) we iterate through the local variable table and find the first entry that contains that slot (line 4 in \bytecoderef{localVarTableDuplicatedSlot}). If the pc of the assignment is not included in the local variable lifespan of the entry (pc 45 $\notin$ [pc: 13, pc: 40], line 4), the next entry with the same slot (pc 45 $\in$ [pc: 44, pc: 94], line 7) will be examined. Checking the entries with the same slot goes on until both the slot and pc criteria fit. Once those criteria are met we can be positive about having the right local variable table entry. We now extract from local variable table the information we need to create a unique identifier for a variable. \ins{The unique identifiers are used for the comparisons explained in} \subsecref{processFlow}. A part of the extracted information is listed below:
...
The exact list of the information we extract can be found in the Appendix \ref{sec:appLocalVars}.
...
Although Javassist does not support the access to information about local variables it provides a way to access information about fields. Javassist allows us to modify an expression in a method body by using the class\linebreak\code{Javassist.expt.ExprEditor}.
...
The full list of the information we extract can be found in the Appendix \ref{sec:appField}.
...
The explanation of the way we do it can be found in the Appendix \ref{sec:appInstrumentation}.
...
The line number \textit{31} is listed twice in the line number table (\bytecoderef{4-3}). This indicates that the method invocation is split into multiple lines in source code.
...
A more complicated situation such as the one shown in the example \coderef{3-1} can arise. In this example the method invocation is situated in the second line unlike the invocation in the previous example. This is interesting because line numbers 127 and 128 are stored in an alternating way in the line number table shown in \bytecoderef{3-3}. The line number table indicates the followings:
...
We tried to insert the code by specifying the exact line number that we want to instrument, \ie the line after the assignment. 
...
How we build the bytecode sequence insert is explained in Appendix \ref{sec:appInstrumentation}.
...
NullSpy is not capable of tracking null assignment in \ins{three} circumstances: if the culprit of a \npe is caused by
\begin{itemize}
	\item an element of a collection and
	\item an object that is a return value of a method invocation and
	\item \ins{when the \npe is triggered in a multithreaded environment.}
\end{itemize}

In situations where an element of a collection causes a \npe, we did not develop a way to refer to that element. We have a rough idea of how to store this information, but it would involve effort that is beyond the scope of this thesis.
Hence, we cannot identify collection elements.

To be able to track null assignment of \mrs we have to statically analyze bytecode and obtain information about that variable. We are not able to statically extract information about an object that is the return value of a method call. For this issue we also have a rough idea of how to deal with it. But again, the effort required would be beyond the scope of this thesis.

\ins{Another situation NullSpy does not support yet is when a \npe is triggered in another thread than the one including the \code{main()} method, the \npe cannot go up the call hierarchy until the \code{main()} method since each thread has its own stack. Because the \npe does not reach the \code{main()} method our inserted \cblock of the \code{main()} method cannot be reached. Thus the \npe cannot handled by NullSpy. The usage of multiple threads is another problem that is not handled in the scope of this work.}
...
The run time of the modified project takes rounded 0.363s longer than the original one. Thus after instrumenting the code, the project results in approximately \textbf{5\%} performance overhead. We claim this overhead is acceptable for the advantages NullSpy offers.
...
In short, we successfully managed to meet the main goal we have set at the beginning of the project. NullSpy is now capable of tracking a \npe to its root and provide the user with more information about its origin without a significant overhead. The most important steps that lead to the success of NullSpy are listed below:
...
For now NullSpy works only on projects that have a clear starting point in the \code{main()} method. NullSpy can be improved to also be applicable to projects that do not have a \code{main()} method, \eg Java-Applets or web applications. For this, a suitable or even customizable way on where to place the try-catch has to be implemented.
...
\section{Bytecode Combinations of \MRs}
\label{sec:bytecodeCombinations}

In the algorithm that identifies the \mr pc-interval we have to split the outermost invocation pc-interval into candidate \mr pc-intervals. Here we discuss how the extraction of all possible combinations of \mrs that are embedded in the outermost pc-interval are extracted. 

We developed a system that checks whether the combinations of specific opcodes are valid. 
\ins{Bear in mind that \textit{aload\_0} represents \textit{this} if we are analyzing a non-static method. If we are analyzing a static method the opcode \textit{aload\_0} refers to the first argument of the method, if there are any, or to the first declared local variable of the method.}
The combinations for identifying the candidate \mr pc-intervals are listed below:  
		...
		\begin{itemize}
			...
			We support only up to two consecutive \textit{getfield} instructions; we have not encountered any case with more than two instructions in row.
		\end{itemize}
...
\ins{To illustrate how this list is used we go through the synthetic bytecode example shown in }\bytecoderef{appMrBytecodeCombiBytecode}. \ins{This bytecode represents the source code shown in }\coderef{appMrBytecodeCombi}.

\ins{The outermost pc-interval of the method invocation \code{move(...)} is <26,33>. When we iterate through the outermost invocation pc-interval we encounter the opcode \textit{``aload.*''} at line 1. We check what comes after this opcode. Again, there is the opcode \textit{``aload.*''} at line 2. With that we look at the list of the possible \mr bytecode combinations and search for \textit{``aload.*''} followed by \textit{``aload.*''}. This expression does not fit into our template which indicates that those two opcodes do not have any relationship with each other. So we can be sure that they are two separate \mr candidates. After that we check the opcode at line 3 and see if the opcodes from lines 2 and 3 fit into our template. For the opcode at line 4 it is the same situation.}

\ins{But when we check the opcode \textit{``getfield''} at line 5 we see that the opcodes \textit{``aload.*''} followed by \textit{``getfield''} fits into our template. With that we know that the combination of the opcodes at line 4 and line 5 represents one \mr candidate.}

\ins{After this process, the outermost pc-interval is split into \{<26,26>,<27,27>,<28,28>,<29,30>\}.}

\ins{We continue this process until the following opcode does not fit into the allowed pattern of the template. As long as the checked opcode fits into our template, we consider them as one possible \mr.}

\ins{For other non-primitive types (\eg Strings) and inlined variable creation we do a similar process. If there is an embedded method invocation, we take the outermost pc-interval of that as a \mr candidate of the external method invocation. The end-pc of the embedded method invocation always comes before the external one in the bytecode sequence. So we will have already figured out the outermost pc-interval of the embedded method invocation before we examine the external one.}

\renewcommand\lstlistingname{Bytecode}
\begin{JVMIS}[numbers=left, caption={Synthetic bytecode example to illustrate the system to split the outermost invocation pc-interval into candidate \mr pc-intervals.}, label={bytecode:appMrBytecodeCombiBytecode}, breaklines=true]
26  aload_1 [figure]
27  aload_2 [x]
28  aload_3 [y]
29  aload_0 [this]
30  getfield AppendixMethodReceiverBytecodeCombi.forwardDirection : java.lang.Integer [17]
33  invokevirtual Figure.move(java.lang.Integer, java.lang.Integer, java.lang.Integer) : void [27]
\end{JVMIS}

\renewcommand\lstlistingname{Code}
\begin{Java}[caption={The source code of \bytecoderef{appMrBytecodeCombiBytecode}.}, label={code:appMrBytecodeCombi}, numbers=none]
figure.move(x, y, forwardDirection);
\end{Java}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%                          Appendix Target Variable          	                 %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\section{\MRs}
\label{sec:appReceivers}
...
\begin{itemize}
	\item \mr id (simple counter which increments on every occurrence)
	\item source code line number of the \mr
	...
	\item is the variable static or not
	\item variable name
	\item statically declared variable type
...
\section{Local Variable Assignments}
\label{sec:appLocalVars}

To find the location of the null assignment NullSpy has to perform several comparisons of the identifiers representing local variables or fields with the identifiers of \mrs. To create those identifiers of local variables we have to collect information about them. We listed part of the information needed to create a identifier of a local variable in \subsubsecref{localVariable}. The full list contains following information:
...
\begin{enumerate}
	...
	We use the term \textit{\textbf{member field}} to describe a field declared in another class than the class under analysis.\\
...
\begin{itemize}
	...
	\item is the field static or not
	\item field name
	\item statically declared field type
...
\begin{itemize}
	...
	\item is the member field static or not
...
After each non-primitive variable assignment we insert bytecode that represents a method invocation. The invoked method is part of the class named\linebreak\code{VariableTester}\footnote{\label{package1}In package \emph{ch.scg.nullSpy.runtimeSupporter}. This package is added to the modified project when the bytecode modification is done.}. 
Depending on the variable (local variable or field) being analyzed at the moment, \ins{a different method is invoked.}

\ins{If we take the source code snippet }\coderef{appDemonstration}\ins{ which is the source code at line 16 from the demonstration example }\coderef{demonstration}{ and decompile the instrumented version of this snippet, it looks the way as shown in }\coderef{appInstrumentedDemonstrationDecompiled}.

\renewcommand\lstlistingname{Code}
\begin{Java}[caption={Source code at line 16 of the NullSpy demonstration example shown in \coderef{demonstration}.}, label={code:appDemonstration}, firstnumber=16]
reader = new BufferedReader(new FileReader(new File("file.txt")));
\end{Java}

\renewcommand\lstlistingname{Code}
\begin{Java}[caption={Decompiled instrumented version of \coderef{appDemonstration}.}, label={code:appInstrumentedDemonstrationDecompiled}, firstnumber=16]
reader = new BufferedReader(new FileReader(new File("file.txt"))); VariableTester.testDirectField("Ideone", "makeReader", "()V", "field", "reader", "Ljava/io/BufferedReader;", "Ideone", 1, reader, 25, 0, 23, 26);
\end{Java}
...
\renewcommand\lstlistingname{Code}
\begin{Java}[caption={This bytecode is inserted after each assignment to a field of the currently analyzed class. It represents the \ins{bytecode added after a field assignment.}}, label={code:bytecodeCreation}, firstnumber=1]
...
This method takes seven java.lang.String objects, one integer, one Object and four integers as arguments. The code snippet \coderef{bytecodeCreation} reproduces these arguments. 

The instruction \textit{``addLdc''} from line 16 to 22 is used to add strings to the bytecode. For adding a boolean we use the integers \textit{``1''} and \textit{``0''} to represent the boolean value \textit{true} and \textit{false} respectively, \ins{as used in Java bytecode}. Hence, line 25 to 29 adds an integer to the bytecode sequence. 

The object, in our case a field, is added to the bytecode sequence by using the method \code{addAload(...)} at line 31 and the method \code{addGetfield(...)} at lines 32 and 33. These lines add the instruction set which loads a field onto the operand stack: \textit{``aload\_0'', ``getfield''}. NullSpy takes the field itself as an argument to evaluate its value in the check-method.
...
With the classes \textit{CodeAttribute} and \textit{CodeIterator} from the Javassist API we can enter our bytecode sequence into existing ones. CodeAttribute represents the bytecode of a behavior and with CodeIterator we can iterate through that bytecode. We obtain a CodeIterator object from the CodeAttribute. So we first move the CodeIterator to the position in the bytecode with \code{codeIterator.move(pc)} where we want to enter our bytecode and then enter our sequence with a call to \code{codeIterator.insert(ourByteCode)}.
