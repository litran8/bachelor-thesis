...
As mentioned before, the bytecode of each method also holds a local variable table which is alist of  information about local variables (lines 13-15 in \bytecoderef{helloWorld}). This table has the information about the \textit{this} reference (if the method is an instance method or a constructor), about method parameters and method local variables, respectively. Example of the \textit{this} reference representation can be seen in the line 14. Each line of this table represents one variable and contains the following:
...
As explained in \charef{introduction}, this project is about providing the user with an additional piece of information next to the common stack trace containing the location of a \textit{null} assignment to the variable that caused the \npe. We use the term \textbf{link} to describe the source code line where the variable that caused the \npe was assigned null.
...
After loading the classes, we first extract data from the class bytecode and then instrument it.
...
To do this, NullSpy looks for the opcodes which match the regex \textit{``invoke.*''}. Each of these opcodes represents one method invocation. There are four invocation bytecode instructions in Java: \textit{invokeinterface}, \textit{invokespecial}, \textit{invokestatic}, \textit{invokevirtual}.
...
To be able to track null assignment of \mrs we have to statically analyze bytecode and obtain information about that variable. We are not able to statically extract information about an object that is the return value of a method call. \del{What value the method invocation returns can only be validated at run time.}\brs{not sure this is true, it could be done by introducing temporary variables, we just did not want to do it}\lt{einfach letzen satz löschen?}
...
The runtime of the modified project takes 0.362734s longer than the original one, this means that after instrumenting the code, the project results in approximately \textbf{5\%} overhead. We claim this overhead is acceptable for the advantages NullSpy offers.
...
\item Collecting data about variable assignments: local variables and fields. For this purpose we statically analyze and instrument the bytecode. We use Javassist to scan bytecode for instructions that represent variable assignments (regexs: \textit{``astore.*''} and \textit{``put.*''}). We extract information about those assignments and pass the collected information to the code we insert after each variable assignment. During run time we check whether the assignment value is null. If it is, we store the information about this assignment.
...
For now NullSpy works only on projects which have a clear starting point in the \code{main()} method. NullSpy can be improved to also be applicable to projects that do not have a \code{main()} method, \eg Java-Applets or web applications.
