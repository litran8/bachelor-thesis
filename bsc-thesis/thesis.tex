...
\chk{Let us now have a look at an example that contains the explained situation, where a method invocation is split into two lines. The example is shown in \coderef{4-1}. 
...
\chk{First, the method invocation \code{getUndoActivity()} at line 128 is executed and then the execution jumps back to line 127 to perform the cast to \code{ChangeConnectionsHandle.UndoActivity}. After the cast, again execution jumps to line 128 where the method \code{getOldConnector()} is invoked.}

\chk{With the default analyses we would obtain <?,?> as the invocation pc-interval, whereas the correct one is <47,54>. 
...
\chk{For complexity reasons we omit the implementation explanation.}
...
Coding in a nice way so that the code does not smell was another challenge due to my lack of experience. Sometimes I tended to create god classes instead of following the single responsibility principle. Therefore, we had to refactor the whole project multiple times, which also took some time. 
