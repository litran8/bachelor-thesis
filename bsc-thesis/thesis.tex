...
We start with an easy \textit{HelloWorld} source code example (\coderef{helloWorld}). The code snippet in the example \coderef{helloWorld} contains the definition of the class \code{HelloWorld} with an instance field named \code{hello} and method \code{say()} which prints the value of the field. The bytecode of the method \code{say()} is represented in the \bytecoderef{helloWorld}. \ins{We omit the bytecode of the class and field definition since it is not relevant for our example.}

\nm{Omit the first part of the bytecode, \ie everything before line 25, reset the counter of the line to 0 instead of 25, and update the text correspondingly.}
...
On the left-side of these instructions we see the \textbf{program counter} (pc) which is a processor register that contains the address (location) of the instruction being executed at the current time. It is also called an \textbf{instruction pointer}. We use these program counters, or pc-s also as an index in the bytecode. The instructions from pc 0 to pc 4 (lines 26-28) in \bytecoderef{helloWorld} represents the source code at line 5 in \coderef{helloWorld}. Therefore we use the term \textbf{pc-interval} to describe a set of instructions that represents a Java expression. This term will play an important role further in the thesis. We use the syntax \textbf{``<,>''} to represent \chg{the}{an} pc-interval\chg{. For the example}{, \eg} just mentioned \del{the} pc-interval <0,4> represents the source code at line 5 in \coderef{helloWorld}. The first number indicates where the pc-interval starts and the latter one where it ends, both inclusive.

If the opcode represents the read from a variable, on its the right side there is the information about the name and type of the variable, \eg line 27. 
In case of an invocation opcode, it shows the behavior/method name, the parameters it takes and the return type, \eg line 31. In \secref{lowLevelOverview} we will see how this information is used.

Each method\ins{ bytecode representation} holds a \textbf{line number table} and a \textbf{local variable table} which are listed underneath the instructions of the method (lines 33-39 in \bytecoderef{helloWorld}). 

\chg{This}{The} line number table\footnote{In the jargon of Javasist, this is also called the line number attribute.} maps the pc to the source code line of the method \del{\code{say()} ( line 25 to 39)}. 
The line number table in \bytecoderef{helloWorld} is represented between the lines 33 and 36: source code line 5 starts with the pc 0 (represented by the line 34), source code line 6 starts with the pc 5 (in line 35) \etc. This\ins{ mapping} does not apply to cases where \chg{an expression}{a statement} is separated into multiple line\ins{s}. In \subsecref{methodReceiverDifficulties} we explain further details about the line number table in a split Java statement case.

\nm{@Lina: rename box to stack trace}
...
Local variable table:
 ...
 [pc: 44, pc: 94] local: e index: \textbf{2} type: java.io.IOException
\end{JVMIS}
...
As mentioned before, the bytecode of each method also holds a local variable table which is an array of the information about local variables (lines 37-39 in \bytecoderef{helloWorld}). This table has the information about the \textit{this} reference (if the method is an instance method or a constructor), about method parameters and method local variables, respectively. \ins{Example of the \textit{this} reference representation can be seen in the line 38.}
Each line of this table represents one variable. It has
...
\ins{In our example, \textit{this} reference has the lexical scope of the whole method, thus pc starts at 0 and ends with 13. \textit{This} reference is the first one in the table, hence it is indexed with 0, and its type is class \code{HelloWorld}.}

(\eg \bytecoderef{localVarTableDuplicatedSlot1}) 

\ins{Let us now look at more complicated example of the local variable table} (\bytecoderef{localVarTableDuplicatedSlot1}). 
Local variables do have a limited lifespan. 
Let us have a look at the source code example \coderef{localVarTableDuplicatedSlot1SourceCode} which contains a method \code{void loadDrawing(String filename)}. The bytecode of this example is shown in the \bytecoderef{localVarTableDuplicatedSlot1}.
This method consists of a \code{try-catch} block. We see that the variable \code{url} (declared at line 4) is only accessible within the \code{try\{\}} block which starts from pc 0 and ends with pc 40.
This local variable is stored at the index 2 in the local variable table in \bytecoderef{localVarTableDuplicatedSlot1}, since the first two indexes occupy \textit{this} reference and the method parameter. Variable \code{url} is created at the pc 13 and is only accessible until pc 40 (excluded). 
...
As soon as a \npe is thrown, we sequentially compare data to get the right location of the null assignment. The comparison-sequence looks as follows (we use the symbol ``\textbf{$\|$}'' for the comparison):
...
At the end of the bytecode modification part we wrap the \code{main()} method into a \textit{try-catch-block}. In the \textit{catch-block} the information about the location where the \npe happened (class name, method name, line number) are extracted from the \npe stack trace. This information is passed on to a static method of the run time supporter package. The method compares the parameters with the stored information about the \mrs. If there is a match regarding the comparison, NullSpy creates a variable identifier with the information about the matched \mr. With this identifier another comparison is performed. This time NullSpy compares the identifier with the information stored about the null assignment. The result will be the stored entry about the null assignment of the variable that caused the \npe. From the entry we extract the line number of the assignment and create the additional link that reveals the location of the null assignment. This link of course is then added to the common \npe stack trace.
