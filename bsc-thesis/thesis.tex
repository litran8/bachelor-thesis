...
\chk{There are other tools which can help developers to debug and to find the root cause of a bug. One of them to mention is the Back-in-time debugger~\cite{Lien08b}. The differences between NullSpy and Back-in-time debugger are:}
...
\chk{Also the Object Flow Analysis could help to find the root cause of a \npe by tracking the object flow~\cite{Lien08d}. The main difference here to NullSpy is that NullSpy tracks the null assignments instead of the object flow.}
...
If the modified bytecode (\bytecoderef{modifiedBytecode}) were to be decompiled, its source code representation \chk{would be as shown in} \coderef{modifiedCodeDecompiled}. At line 2 of this code, we can see the changes, \ie the inserted line of code. However, the actual result is the inserted bytecode represented by the pc-interval <0,5> in the modified bytecode (\bytecoderef{modifiedBytecode}).
...
\chk{As Jad illustration we decompile the bytecode shown in \bytecoderef{initialBytecode} of the source code example \coderef{initialCode} and indicate the differences between the decompiled and the original version of the source code.
The result of the decompilation is shown in \coderef{decompiledBytecode}.}
...
\chk{The general approach of NullSpy is to statically analyze a project in which a \npe occurs and instrument the bytecode. NullSpy statically analyzes the project to extract every data from bytecode about variables, \ie the name of the variable, the source code line number where it is located \etc We instrument the code to get the value stored in those variables.} 
...
%-------------------------------------------------------------------------------
\subsection{Process Flow\chk{ - NullSpy Process Overview}}
...
The first step in the modification part is to statically gather information about the \mrs \chk{ and the variable assignments}. We use the term \textit{\textbf{\mr}} to refer to variables on which methods are invoked, or whose fields are accessed.
...
\chk{For every non-primitive variable assignment we also perform static analysis to collect information about the variable. The information again is needed to create its unique identifier.}

The second step is to instrument the assignments. \del{For every non-primitive variable assignment
we collect information about the variable, needed to create its unique identifier.} After the assignment we insert code which represents a call to the \textbf{run-time supporter} package with the variable's identifier. The run-time supporter package checks at run-time whether the variable is assigned null. If this is the case we store the information about this variable.
...
If the \mr is actually the return value from another method invocation, it is not apparent where exactly the \mr is situated in bytecode. More about the challenges we encountered can be found in \subsecref{methodReceiverDifficulties}.
...
Let us assume that we have a local variable assignment where its \textit{``astore\_2''} opcode is located at pc 45 (\bytecoderef{localVarAssignment}). In this example we can extract slot 2 from the opcode \textit{astore\_2}. After extracting the slot (index: 2) we iterate through the local variable table and find the first entry that contains that slot (line 4 in \bytecoderef{localVarTableDuplicatedSlot}). If the pc of the assignment is not included in the local variable lifespan of the entry (pc 45 $\notin$ [pc: 13, pc: 40], line 4), the next entry with the same slot (pc 45 $\in$ [pc: 44, pc: 94], line 7) will be examined. Checking the entries with the same slot goes on until both the slot and pc criteria fit. Once those criteria are met we can be positive about having the right local variable table entry. We now extract from local variable table the information we need to create a unique identifier for a variable. \chk{The unique identifiers are used for the comparisons explained in \subsecref{processFlow}.} A part of the extracted information is listed below:
...
\chk{For complexity reasons we only explain the key idea behind this system. For each invocation we look for the end-pc. With that we go back and forth the line number table of the method including the invocation and check whether there is another entry with the same source code line number as the end-pc matches. In examples like the one shown in \coderef{3-1} we also have to pay attention that the end- and start-pc of the outermost pc-interval do not have to match the same source code line number.}

\chk{Another difficulty with the split invocations is that even with the system of getting the outermost pc-interval we sometimes still get a smaller one than the one representing the invocation. This happens because we cannot get more information out of the line number table. For this reason we have to take the number of arguments the invocation expects for help to complete the outermost pc-interval. If the outermost pc-interval only includes $N$ arguments instead of $N+1$ we have to add another candidate pc-interval to the outermost pc-interval with the system explained in \ref{sec:bytecodeCombinations}.}
...
NullSpy is not capable of tracking null assignment in two circumstances: if the culprit of a \npe is caused by
\begin{itemize}
...
\chk{Bear in mind that \textit{aload\_0} represents \textit{this} if we are analyzing a non-static method. If we are analyzing a static method the opcode \textit{aload\_0} refers to the first argument of the method, if there are any, or to the first declared local variable of the method.}
...
\chk{To illustrate how this list is used we go through the synthetic bytecode example shown in \bytecoderef{appMrBytecodeCombiBytecode}. This bytecode represents the source code shown in \coderef{appMrBytecodeCombi}.}

\chk{The outermost pc-interval of the method invocation \code{move(...)} is <26,33>. When we iterate through the outermost invocation pc-interval we encounter the opcode \textit{``aload.*''} at line 1. We check what comes after this opcode. Again, there is the opcode \textit{``aload.*''} at line 2. With that we look at the list and search for \textit{(aload.*).(aload*)}. This expression does not fit into our template which indicates that those two opcodes do not have any relationship with each other. So we can be sure that they are two separate \mr candidates. After that we check the opcode at line 3 and see if the ``concatenation'' of the opcodes from lines 2 and 3 fits into our template. For the opcode at line 4 it is the same situation.}

\chk{But when we check the opcode \textit{``getfield''} at line 5 we see that the ``concatenation'' \textit{(aload.*).getfield} fits into our template. With that we know that the combination of the opcode at line 4 and the one at line 5 represents one \mr candidate.}

\chk{After this process, the outermost pc-interval is split into \{<26,26>,<27,27>,<28,28>,<29,30>\}.}

\chk{We do this checkings until the following opcode does not fit into the allowed pattern of the template. As long as the checked opcode fits into our template, we consider them as one possible \mr.}

\chk{For other non-primitive types (\eg Strings, variable creation) we do it in a similar way. If there is an embedded method invocation, we take the outermost pc-interval of that as a \mr candidate of the external method invocation. The end-pc of a embedded method invocation always comes before the external one in the bytecode sequence. So we will have already figured out the outermost pc-interval of the embedded method invocation before we examine the external one.}
...
Depending on the variable (local variable or field) being analyzed at the moment, different bytecode is constructed \chk{that represents a method call which takes different number of arguments. The reason for that is because we need less information about local variables than fields to uniquely identify them. So we pass less arguments to the method call inserted after a local variable assignment than the one inserted after a field assignment.}

\chk{This inserted method checks whether the value of the variable received as an argument is null. If the value is null, NullSpy stores the information about that variable which is also passed as argument to this method.}
...
\renewcommand\lstlistingname{Code}
\begin{Java}[caption={This bytecode is inserted after each assignment to a field of the currently analyzed class. It represents the \chk{bytecode added after a field assignment.}}, label={code:bytecodeCreation}, firstnumber=1]
...
\chk{If we take the source code snippet \coderef{appDemonstration} which is the source code at line 16 from the demonstration example \coderef{demonstration} and decompile the instrumented version of this snippet, it looks the way as shown in \coderef{appInstrumentedDemonstrationDecompiled}.}
