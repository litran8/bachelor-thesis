...
The indexes of the the local variable table are important because we use them to get the right local variable we are interested in. How we make use of the indexes to find the aimed variable is explained in \subsecref{variable}.
...
In this section we explain how to get information about the \mr needed to construct a unique identifier. A small synthetic example (\coderef{methodReceiverAlgDemoCode}) makes is easier to understand. \Mrs in the example are the variables \code{fToolButton} at line 7, \code{fToolButton.myIcon} at line 8 and \code{activePanel} at line 11.
...
We use the library Javassist to partially extract data from bytecode. Unfortunately, Javassist does not provide the functionality to directly get all the information we need about the \mr, thus we need to do it manually. The exact information we need is listed in \listref{methodReceiverInfo} and it is stored in an external \textit{csv} file due to overhead minimization.
...
Let us have a look at line 11 of the example source code shown in \coderef{methodReceiverAlgDemoCode} and its bytecode presented in \bytecoderef{methodReceiverAlgDemoBytecode}. In this example we want to extract the pc-interval of the \mr \code{activePanel}.
...
With the line number table (shown in \bytecoderef{lineNrTable}) and the pc we can obtain the line number. In the paragraph before we looked a variable assignment with pc 45 as an example, so the local variable assignment starts at line 64 according to the line number table.
...
\label{subsec:field}
Although Javassist does not support the access to local variable it provides a way for fields. Javassist allows to modify an expression in a method body with the class \code{Javassist.expt.ExprEditor}. 
...
What it does is to scan the bytecodes on instructions like \textit{``putfield''} for instance or \textit{``putstatic''} for static fields.
...
As already mentioned earlier, every time when encountering an assignment either of a local variable or a field, NullSpy inserts bytecode to check whether the variable is assigned null. If the variable is assigned null, NullSpy stores all the data about the variable. How we insert bytecode is described next.
