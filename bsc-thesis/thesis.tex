...
The combinations for identifying the candidate \mr pc-intervals are listed below:  
		...
		\begin{enumerate}
			\item aload\_0.getfield // access to any kind of field of \textit{this} object, or access to public field of argument/local variable \todo{update}
			\item aload\_0.getfield.getfield.getfield // access to a public field
			\item aload\_0.getfield.getfield.getfield.getfield // we support only until three consecutive \textit{getfield} instructions; we have not encountered any case with more than three instructions in row
		\end{enumerate}
...
If the following opcode after the first \textit{``aload.*"} is another \textit{``aload.*"} we can be sure that they are two separate \mr candidates because these \textit{``aload.*"}s represent two variables that does not have any relationship with each other.
...
We explained that we extract \mr information to create its unique identifier for the several comparisons which have to be done to find out the location of the null assignment of the \mr. In the subsection \nm{which one?} where we describe the algorithm for the extraction of \mr pc-interval, we gave a hint of what kind of information we need to create this unique identifier. The complete list of what data we store about the \mr is listed next:
...
In the \todo{section} we listed a part of the data which we actually extracted from field assignment bytecode to create the unique identifier for the field. 
