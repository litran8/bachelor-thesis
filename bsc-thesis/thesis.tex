...
The \code{open()} method calls the methods \code{createIconkit()} at line 7 and \code{setTool()} at line 9. The former will \chg{initiate}{initialize} the field \code{fManager} (line 3) and the latter will cause the \npe.

By calling \code{createIconkit()} the \ugh{field is assigned to the method} \code{getIconkitManager()} at line 14 which just returns the \textbf{null} value at line 19.

In \ins{the} method \code{setTool()} (line 22) the \npe is finally \chg{thrown}{caused} by the attempt to access the field at line 24. The resulting \npe stack trace is shown in \boxref{stackTrace}:
...
At this point we introduce our project named \textit{\textbf{NullSpy}} which supports the developers in situations discussed previously. \del{In other words }the main goal of NullSpy is to take a step towards minimizing the time spent debugging \npes. The intention behind NullSpy is to, once an unhandled \npe has occurred, present the developers the exact location of the null assignment next to the ordinary stack trace as shown in \boxref{stackTrace2}.
...
\section{Bytecode}
\label{sec:bytecode}
The backbone of NullSpy is analysis and modification of Java bytecode. Java bytecode is an abstract machine language that the stack-based Java virutal machine (JVM) can understand and execute. \ugh{The instructions JVM executes are \eg \textit{aload\_0}, \textit{putfield}, \textit{invokevirtual} and so on}\brs{pointless, you should say that the JVM keeps a stack and that each instruction modifies the stack, that the instructions have an opcode and a string representation.}.

Since bytecode plays an important role in our project, we will describe the Java bytecode and explain some terms we will use often in this thesis. We start with an easy \textit{HelloWorld} source code \ins{example}(\coderef{helloWorld}) and its \ins{bytecode (}\bytecoderef{helloWorld} \ins{)} \del{example} \del{to make it easier to understand}. \brs{Is this really byte code? Looks to me like a human readable presentation of the bytecode or something, i doubt the JVM parses this}
...
We are going to look only at the bytecode of method \code{say()}. \chg{Those}{The} green words in \bytecoderef{helloWorld} are called Java bytecode instructions. We will also use the term \textit{opcode} as a synonym for those instructions since it is a number that represents the operation to perform.

On the left-side of these instructions we see the \textit{program counter} (pc) which is a processor register that \ugh{indicates where a computer is situated in its program sequence}\brs{what?}\lt{better?}. It is also called instruction pointer. We use these pcs also as an index in the bytecode. The instructions from pc 0 to pc 4 in \bytecoderef{helloWorld} represents the source code at line 5 in \coderef{helloWorld}. Therefore we use the term \textit{pc-interval} to describe an instruction-set that represents \eg a method invocation, an assignment \etc. This term will play an important role further in the thesis.
...
Underneath the instructions\brs{why are there no line numbers in bytecode? it would help here} we find the \textit{line number table}. It maps the pc to the source code line. The numbers at the end of the lines in \boxref{stackTrace} indicate the lines of code that caused the \npe. The information the \textit{line number table} holds is used to recover where the null was assigned. In \secref{lowLevelOverview} we will discuss more about the \textit{line number} and \textit{local variable attribute}.
...
\begin{enumerate}
	\item First a \textit{ClassPool} object that controls bytecode modification \del{with Javassist} is obtained. With the \textit{ClassPool} \ins{object} a class file (``.class'') can be read on demand for constructing a \textit{CtClass} object.

	\item The class \textit{CtClass} (compile-time class) represents the class file\ins{.}\chg{meaning}{This means} that all manipulations are performed on the \textit{CtClass} object. We obtain a reference to the \textit{CtClass} object representing the \code{test.Rectangle} class by invoking the \code{get()} method on the \textit{ClassPool} instance.

	\item In this example the only bytecode modification done is changing the superclass of \code{test.Rectangle} to \code{test.Point}.\ins{This does not serve any purpose and is only illustrative}
...
\figref{javassistModules}\footnote{Figure taken from \href{https://blog.newrelic.com/2014/09/29/diving-bytecode-manipulation-creating-audit-log-asm-javassist/}{blog.newrelic.com}.} gives an overview of how the main part of bytecode manipulation with Javassist is built up. The \textit{ClassPool} is simply a container of multiple \textit{CtClasses}. As described before \textit{CtClass} represents \ins{a }class file\del{s} on which modifications are done. Like typical classes, it can hold compile-time fields, constants or methods. Javassist is capable of adding or modifying classes, behaviors, fields, method invocations, local variables \etc But in our case we mainly address the manipulation of behaviors. It is possible to insert additional source code at the beginning of the method body, at the end or at a specific line. The next example shows how to add code using Javassist.
...
With \coderef{bytecodeModifier} we obtain a \textit{CtClass} object \code{cc} at line 5\brs{update lines pls} which represents the class to be modified. At line 7 we get the method we want to modify by adding some source code: \code{System.out.println(``This is the inserted code.'');}. Javassist takes a \textit{java.lang.String} object, compiles it automatically and adds it into the bytecode at the specified location, in our situation at the beginning of the \code{main()} method.

\coderef{instrumentedCodeDecompiled} is the result \ie the \code{main()} method equivalent to what we achieved with Javassist. At line 2 we see the inserted code.

\brs{Um... no... The result is the bytecode, the code is what we wish to achieve, or, as you said in the caption, a decompilation of the result.}
...
The example \bytecoderef{jad} is decompiled by JAD and the result is the source code right under the example.\brs{is it? don't count on positions when using float}
...
\chapter{NullSpy}
As explained in \charef{introduction}, this project is about providing the user with an additional stack trace \brs{not true, in chapter 1 (box 1.2) you just showed the location not the stack trace} containing the information on where the null is assigned to the variable that caused the \npe.
...
The general approach of NullSpy is to statically analyze a project and instrument the bytecode. NullSpy is a console application which takes two \chg{parameters}{arguments}. The first argument to NullSpy is the path to the folder containing the compiled class files of the original project and the second one is the destination where the user wants to store the modified \chg{project}{class files}.

We provide the option to choose the destination \del{is because} in case the developer does not want to overwrite the original project with the modified one. This means that the updated bytecode can be stored in another location, keeping the original class files intact. Of course the original project can be replaced by the modified one too. But the developer has to think carefully about it because once replaced, the additional bytecode stays there and the execution time is slightly different than it was before. To get rid of the additional bytecode, the developer has to recompile the source code to get the original class files when needed. \ugh{This has to be done every time when NullSpy is used on the project}\brs{what? you mean every time the developer overrides the original class files and wants them back, not every time one runs NullSpy?}.
...
Loading is done by traversing all the subfolders of the folder given as a parameter, and extracting all the \textit{.class} files. NullSpy modifies the class file directly when it is loaded. As soon as the modification is done, the class file is stored to the destination folder \chg{which is represented by}{\ie} the second parameter of NullSpy. 

Storing is done by recreating the structure from the source folder in the destination folder and storing the modified class files \chg{similarly}{using the Javasist API} as explained in \secref{javassist}. \del{In secref{javassist} we store the modified class file on a local disk. This way the original class file remains intact. In our case we write the modified class file in the destination folder not only on a local disk. This way the original project can be overwritten as explained before.} \brs{what? not only on a local disk? what does that even mean}
...
As mentioned in \subsecref{processFlow}, a big part of NullSpy is extracting information from bytecode. One of the item we are interested to get information about is the \mr. A small syntactic example (\coderef{methodReceiverExample}) makes is easier to understand. \Mrs in \coderef{methodReceiverExample} are the two variables \code{var} at line 8 and \code{field} at line 9.
...
We use the library Javassist (\secref{javassist}) to extract data from bytecode. Unfortunately, Javassist does not provide the functionality to directly get information about the \mr. The exact information we need is listed later in this section. To put it another way, with Javassist we cannot directly extract the \mrs from bytecode. We need the exact pc-interval of the \mr to extract the right data from bytecode. That is why we implemented our own algorithm to get the \mrs. While iterating through the bytecode NullSpy looks for the opcodes which matches the regex \textit{``invoke.*''} and uses the algorithm to get the right pc-interval. The algorithm contains following steps:
...
\renewcommand\figurename{Algorithm}
	\begin{boxedminipage}{\textwidth}
		\begin{enumerate}
\itemsep8pt
      \item \textbf{Getting the pc-interval of method invocation:}\\
			The bytecode of a simple method invocation is constructed as following: first the \mr is pushed on the operand stack, than the parameters the method call needs and last the opcode \textit{``invoke.*''} is pushed onto the stack. The pc-interval of a method invocation contains all the pcs starting from the start-pc of the \mr up to the pc of the corresponding opcode \textit{``invoke.*''}. The use of the term corresponding is explained in step 4.
      \item \textbf{Storing all possible \mr intervals within the interval of step 1 into an ArrayList}\\
			Since we cannot get the exact pc-interval of the \mr yet or we even do not know whether there exists\footnotemark one for the method invocation we are looking at, NullSpy stores every pc-interval which can be a \mr candidate within the pc-interval gained in step 1. Possible \mr candidates are the parameters and the \mr itself. If there is a \mr its pc-interval is stored, the pc-interval of each parameter the method invocation takes are also stored into an ArrayList. Every pc-interval receives the roll of a possible \mr candidate. Parameters can also be another method invocation not only simple variables.
			\item \textbf{Getting the number of parameters, the method invocation takes}\\
			Javassist offers the functionality to count the number of parameters a method invocation has by analyzing its signature. The signature is a \textit{String} that contains the parameter and the return type of the method call.
			\item \textbf{Traversing back the ArrayList the amount of parameters}\\
			To be sure not to mistaken a parameter as the \mr we traverse back the ArrayList the number of parameters the method invocation takes. Traversing back is necessarily to detect nested method invocations. The meaning of nested invocations is \eg a method invocation that has a parameter which is again a method invocation.
			\item \textbf{Result: \mr interval (pc-interval)}
   \end{enumerate}
	\end{boxedminipage}
	\caption{\Mr algorithm.\brs{very unclear, I have no idea what all this means}\lt{clearer now?}}
	\label{alg:methodReceiverAlg}
\end{figure}
\footnotetext{Static method invocations do not have \mrs.}
...
We are only looking at line 3 in \coderef{methodReceiverAlgDemoCode}.
...
\renewcommand\figurename{Example}
	\begin{boxedminipage}{\textwidth}
		\begin{enumerate}
			...
			\item Result: \newline 144 = \code{activePanel} = \mr
		\end{enumerate}
	\end{boxedminipage}
...
In step 1 we had troubles getting the right interval of the \mr. By only statically analyzing the bytecode it is unapparent where exactly the \mr is situated in bytecode. More about the challenges we encountered can be found in \subsecref{methodReceiverDifficulties}.
...
In case of the simple stand-alone \textit{invokestatic} instruction we do not have a \mr because static method invocations do not have a \mr. Like this NullSpy ignores the instruction \textit{invokestatic}. But if the static method call is a parameter of another method call NullSpy wraps it as a \mr candidate. We do this because we traverse back the list that holds all possible \mr pc-intervalls as explained in \algref{methodReceiverAlg}.

\brs{the algorithm is very poorly explained, i still have no idea how you get from pc-interval to any useful information}\lt{see next paragraph}

As soon as we have the \mr pc-interval we can extract information about the \mr from the bytecode. By calling the method \code{constantPool.getFieldrefName(index)} we get the name of the \mr. The parameter \code{index} is obtained by \code{codeIterator.u16bitAt(pc+1)}. To be able to get the parameter \code{index} we need the exact pc-interval of the \mr.

We store the data we gathered about the \mr in an external \textit{csv} file due to overhead minimization. The following list shows the information which is stored about the \mr.
...
The section before we were looking for instructions which matches the regex \textit{``invoke.*''} to detect \mrs in bytecode. This time we are looking for the regexs \textit{``astore.*''} and \textit{``put.*''}. The former refers to local variable assignments and the latter to field assignments. Unlike the information collection about the \mr in \subsecref{methodReceiver} the data about the variables are stored in a HashMap.
...
Unfortunately, Javassist does not provide any support for gaining information about local variables. This is why to get the needed data we had to understand how the bytecode is constructed. At this point we  give a small bytecode introduction\brs{shouldn't this be in the technical background?}\lt{thought this is more difficult to understand and always jumping is not comfortable}.
...
Each method of a class file does not only hold a local variable table but also another attribute called line number attribute. This is the mapping table from pc to source code line number. That means after getting the pc of the local variable assignment we can obtain the right line number. The line number is essential for constructing the link which will be added to the common exception stack trace.
...
The syntactic example \coderef{fieldExample} shows how the instructions \textit{``putfield''} and \textit{``putstatic''} are used:
