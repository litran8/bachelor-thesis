...
The last target for possible future work is to transform NullSpy into an Eclipse plug-in project, which would allow a simpler way to start tracking null assignments.
...
\ins{But when we check the opcode \textit{``getfield''} at line 5 we see that the opcodes \textit{aload.*} followed by \textit{getfield} fits into our template. With that we know that the combination of the opcode at line 4 and the one at line 5 represents one \mr candidate.}
