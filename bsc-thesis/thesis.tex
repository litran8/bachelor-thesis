...
\begin{quotation}
\noindent
A previous study found \del{out} that \npe is the most frequently occurring \del{and difficult to debug }exception in Java projects. \todo{Nevena: In the introduction, when this is elaborated, make sure there are some references to support this claim.} \ins{Also, }It is difficult to debug because a developer is only provided with a stack trace to the line of code where the exception was thrown. This only gives insight into the effect of the fault but not into its cause.
...
\Npe is a commonly occurring \rte in object-oriented languages. \todo{Add some references}\lt{falls noch zeit übrig bleibt, werde ich noch mehr danach suchen. bis jetzt noch nichts gefunden.}\brs{remove the todo, we clearly do not have a reference for this}In most OOP languages there is a special \textbf{null} value that is assigned to references in order to indicate that the reference does not refer to an object. A \npe is caused by invoking a method or accessing a field through a null value reference, \ie a reference with the null value.
...
A more complicated way a \npe can be triggered is when it involves variables with larger scopes such as class members. \coderef{npeExampleFieldAccess}\textsuperscript{\ref{codeTaken1}}\brs{footnote not on the same page, duplicate the footnote} shows such a situation. This code snippet defines the class named \code{DrawApplication} which has the field \code{fManager} (declared at line 3). This class contains four relevant methods: \code{open(...)}, \code{createIconkit()}, \code{getIconkitManager()} and \code{setTool(...)}.
...
In \stacktraceref{stackTrace} we can see that the stack trace points only to the line 24, where \npe occured, and it does not track to the location where the variable is assigned the null value. We call this location the \textbf{root of an exception}. That means the Java developer has to debug their way to the exception root.
...
%\footnote{In the jargon of Javasist, this is also called the line number attribute.} 
maps source code line of the method with the pc which indicates the beginning of the bytecode to the corresponding source code line. The line number table in \bytecoderef{helloWorld} is represented between the lines 9 and 12: source code line 5 starts with the pc 0 (as shown on line 10), source code line 6 starts with the pc 5 (in line 11) \etc This mapping does not apply to cases where a statement is separated into multiple lines.
...
As mentioned before, the bytecode of each method also holds a local variable table which is a\chg{n array of the}{list of } information about local variables (lines 13-15 in \bytecoderef{helloWorld}). This table has the information about the \textit{this} reference (if the method is an instance method or a constructor), about method parameters and method local variables, respectively. Example of the \textit{this} reference representation can be seen in the line 14. Each line of this table represents one variable and contains the following:
...
In our example, \textit{this} reference has the lexical scope of the whole method, thus pc starts at 0 and ends with 13. \textit{This} reference is the first one in the table, hence it is indexed with 0, and its type is \chg{class}{the} \code{HelloWorld} \ins{class}. The indexes of the the local variable table are important because we use them to get the right local variable we are interested in.
...
So, \chg{onward the}{after} pc 40, slot 2 is free again. 
...
As explained in \charef{introduction}, this project is about providing the user with an additional \chg{link}{piece of information} next to the common stack trace containing the location of a \textit{null} assignment to the variable that caused the \npe. We use the term \textbf{link} to \chg{address}{describe} the \ins{source code }line \del{which includes the information }where the variable that caused the \npe was assigned null\del{, and which we add to the common stack trace}.
...
We provide the option to choose the destination in case the developer does not want to overwrite the original project with the modified one. This means that the updated bytecode can be stored in another location, keeping the original class files intact. Of course the original project can also be replaced by the modified one. To \chg{sweep away}{remove} the instrumented bytecode, the developer\ins{ just} has to recompile the source code \chg{to}{and} get the original class files when needed.
...
\begin{enumerate}
	\item \del{Input: }Load compiled class files of the project into NullSpy.
	\item \del{Modification:} The modification part deals with the bytecode instrumentation and adding modules to the project that support the inserted code.
	\item \del{Output:} Store modified class files to the chosen output folder.
\end{enumerate}

Loading is done by traversing through all the subfolders of the folder whose local path was given as a first parameter, and extracting all the \textit{.class} files. NullSpy modifies the class file directly when it is loaded. As soon as the modification is done, the class file is stored at the destination folder \ie the folder whose path is the second parameter of NullSpy. The structure of the source folder is preserved \del{also }at the destination folder.
...
After \chg{class loading}{loading the classes}, we first extract data from the class bytecode and then instrument it.
...
\chg{The way how we extract and store all those data are discussed in the following sections.}{The following sections describe the process of extracting and storing this data.}
...
To do this, NullSpy looks for the opcodes which match the regex \textit{``invoke.*''}. Each of these opcodes represents one method invocation. There are exactly five invocation bytecode instructions in Java: \textit{invokedynamic}, \textit{invokeinterface}, \textit{invokespecial}, \textit{invokestatic}, \textit{invokevirtual}. \lt{inovkedynamic weglassen und einfach sagen, dass Java nur 4 invoke opcodes hat?}
...
The \textbf{start-pc} of the method invocation pc-interval is the pc at which \del{begins }the bytecode \del{which} \chg{represents}{representing} the source code line containing the invocation \ins{begins}. Finding the start-pc is done by using the line number table of the method. We compare the \textbf{end-pc} with the entries of the line number table and take the last entry which has the smaller pc than the \textbf{end-pc}. The line number table of the example bytecode is shown in \bytecoderef{mrBytecodeExampleLineNumberTable}. 
...
Let us assume that we have a local variable assignment where its \textit{``astore\_2''} opcode is located at pc 45 (\bytecoderef{localVarAssignment}). In this example we can extract slot 2 from the opcode \textit{astore\_2}. After extracting the slot (index: 2) we iterate through the local variable table and find the first entry which contains that slot (line 4 in \bytecoderef{localVarTableDuplicatedSlot}). If the pc of the assignment is not included in the local variable lifespan of the entry (pc 45 $\notin$ [pc: 13, pc: 40], line 4), the next entry with the same slot (pc 45 $\in$ [pc: 44, pc: 94], line 7) will be examined. Checking the entries with the same slot goes on until both \chg{criteria}{the} slot and pc \ins{criteria} fit. Once those criteria are met we can be positive about having the right local variable table entry \del{to extract the information needed to create a unique identifier for the variable}. We now extract from local variable table the information we need to create a unique identifier for a variable. A part of the extracted information is listed below:
\begin{itemize}
...
On the other hand, by using the pc of the assignment (45) and line number table (shown in \bytecoderef{lineNrTable}) we obtain the source code line number of the assignment. As we can see in the line number  table, the local variable assignment starts at source code line 64.
...
The line number table entries \ins{in Bytecode X.Y} indicate the followings:
\begin{itemize}
...
The reason \del{why }we decided to use Javassist for \del{our }NullSpy \del{project} was that it allows us to instrument code \chg{at the}{using the} source\ins{-code}-level API.

One big problem we encountered was \chg{the insertion}{inserting} \del{of the }code \chg{if}{when} a variable assignment is the last line of code before a closing curly bracket\del{ \code{\}}}. 
...
Let us look at a\chg{ situation }{n example }where\del{, for example, }the local variable \code{var} is assigned a value at the end of the \code{if-body} \ins{block} at line 3 in \coderef{bytecodeAdaptationExample_1}.
We want to \chg{add }{insert }the \ins{assignment checking }code \del{for assignment check} just after, at line 4\brs{make the example correct java by making line 4 be a comment that says "We want to insert code before this line". Same with the next example saying "We accidentally inserted code here"}. 
...
\nm{I vote to remove this part of the appendix completely. Boris, your opinion?} \brs{Leave it}
How we build the bytecode sequence insert is explained in Appendix \ref{cha:appInstrumentation}.

\del{There are more problems during the implementation of NullSpy but we mentioned the most troublesome ones in this subsection.}
...
To be able to track null assignment of \mrs we have to statically analyze bytecode and obtain information about that variable. We are not able to statically extract information about an object that is the return value of a method call. What value the method invocation returns can only be validated at run time.\brs{not sure this is true, it could be done by introducing temporary variables, we just did not want to do it}
...
{JHotDraw\footnote{\protect\url{http://www.jhotdraw.org/}}} also served to check whether the logic of the bytecode manipulation behind NullSpy is working as desired. JHotDraw is an open-source Java GUI framework for technical and structured Graphics. It is big enough to get reliable numbers and it provides many different cases we had to take care of in NullSpy. \del{Also thanks to Nevena Milojkovi\'{c} and her experience with the combination Javassist and JHotDraw we as well decided to test NullSpy on it.}

%===============================================================================
\section{Execution Time Difference}
\label{sec:execTimeDiff}

JHotDraw already provides an Ant buildfile \textit{built.xml} that packs the project into an executable jar file. We modified the buildfile \del{a little }so that it also creates a jar file out of the \chg{modified project stored in the destination the user has chosen before}{class files instrumented by NullSpy}. The steps to create an executable jar of the modified project are followings: load project, modify project, store modified project, \chg{build project that creates}{crate} jar file of the original project and one of the modified one. We then \del{simulate the terminal with a Java class to} run the tests included in each jar file thirty times, \chg{each time it returns the}{recording the} execution time. At the end we calculate the average time for the original and the modified project. The the execution times are listed in \tabref{executionTime} and the average times are shown in \tabref{averageTime}.
...
The runtime of the modified project takes 0.362734s longer than the original one, this means that after instrumenting the code, the project results in approximately \textbf{5\%} overhead. \chg{This small overhead is quite negligible.}{We claim this overhead is acceptable for the advantages NullSpy offers.} 
...
We want to demonstrate \del{on a real project }how \del{the project} NullSpy helps the user \del{to} find the real culprit that leads to a \npe. 
...
\item Collecting data about variable assignments: local variables and fields. For this purpose we statically analyze and instrument the bytecode. We use Javassist to scan bytecode for instructions that represent variable assignments (regexs: \textit{``astore.*''} and \textit{``put.*''}). We extract information about those assignments and pass the collected information to the code we insert after each variable assignment. During run time we check whether the assignment value is null. If \chg{this}{it} is we store the information about this assignment.

\item NullSpy handles the uncaught \npe. \del{That is why }we wrap up the \code{main()} method with a \cblock. In this \cblock we extract details of where the \npe occurred from the \npe stack trace. With this data and the identifier for \mrs and variable assignments we perform several comparisons to find the location of the null assignment\del{ which is the goal of NullSpy}. 
\end{enumerate}
...
After the implementation of NullSpy we validated it with help of the project JHotDraw. First we measured the time how long JHotDraw takes to execute all its tests without \del{touching}{modifying} the bytecode. After that we instrumented the project and again measured the execution time. The result is a performance overhead of approximately 5\%. So NullSpy could help the developer finding the null assignment without a big impact on the performance.
...
As mentioned in \secref{limitations}, if the culprit that caused the \npe is an element of a collection or the return object of a method invocation, NullSpy cannot track the null assignment of the culprit variable. \del{We intend to improve this.}\brs{no we don't} \ins{This can be overcome by additional instrumentation of the bytecode to keep track of individual elements of collections and by inter-procedural analysis of the byte code}

%-------------------------------------------------------------------------------
\subsection{Track \npe root for all projects}
For now NullSpy works \ins{only} on projects which have a \chg{the }{clear} starting point\chg{, \ie}{in} the \code{main()} method. NullSpy can be improved to also \chg{cover up}{be applicable to} projects that do not have a \code{main()} method, \eg Java-Applets or web applications.
...
\del{For now, we can only think of these future work that could improve NullSpy to give it more value.}
...
NullSpy is based on the instrumentation of code, that means the core work of NullSpy is to modify bytecode (with Javassist). To be able to modify bytecode, I had to learn how bytecode is constructed and to get familiar with Javassist. I went through a good Javassist tutorial\footnote{\url{https://github.com/jboss-javassist/javassist/wiki/Tutorial-1}} to learn how Javassist is used to instrument code mostly on source-code level but also a bit on bytecode-level.

Due to the problem of extracting information about \mrs, I spent a lot of time finding a way how this could be done. It was quite interesting to invent an algorithm that \chg{fullfills}{fulfills} a more complex task.
...
NullSpy is a program which helps Java developers \del{to} find the root of \npes by providing an additional link next to the common stack trace. The key idea behind NullSpy is to help developers \del{to} save time \ins{when} fixing bugs which \chg{are caused by}{manifest as} \npes. This approach tries to provide the service mentioned \chg{by}{while} keeping the overhead \chg{to}{at} a minimum. To demonstrate how NullSpy works, this chapter serves as a small tutorial that takes the example shown in \coderef{demonstration} as the testing code to show the feature\ins{s} of NullSpy.
...
\begin{enumerate}
	...
	\item Checkout \del{the project }NullSpy from \url{https://github.com/litran8/nullpointer-javassist} and import it into eclipse
...
In the algorithm \chg{about finding}{which identifies} the \mr pc-interval we have to split the outermost invocation pc-interval into candidate \mr pc-intervals. Here we discuss \del{about }how the extraction of all possible combinations of \mrs which are embedded in the outermost interval is done. 

We developed a system that checks whether the combinations of specific opcodes are valid. 
Bear in mind that \textit{aload\_0} represents \textit{this} \del{object }if we are analyzing non-static method, or first argument or declared local variable of the method, if we are analyzing a static method.
...
\begin{itemize}
	...
	\item attribute index, if the variable is a local variable\del{, else empty string}
\end{itemize}
