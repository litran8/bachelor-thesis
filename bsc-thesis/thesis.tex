...
\chk{There are other tools which can help developers debug and find the root cause of a bug. Most notably the Back-in-time debuggers~\cite{Lien08b}. The main differences between NullSpy and Back-in-time debuggers are:}
...
\chg{The call to \code{readNumbers()} at line 8 is not important for our demonstration; that is why we skip it in our analysis.}{The call to \code{readNumbers()} at line 8 reads the numbers from the file ``file.txt'' at line 26 and stores them into an \code{ArrayList} at line 27. The list is then returned at line 29. If the variable \code{reader} is assigned null before, the read action at line 26 will trigger a \npe. In our example this is the case. The \npe goes up the call hierarchy until the \code{main()} method.}
...
public class Ideone {
	...
	// NPE triggered here!
	private static List<Integer> readNumbers() throws NumberFormatException, IOException {
...
\renewcommand\figurename{Stack trace}
	\begin{boxedminipage}{\textwidth}
		...
		Exception in thread "main" \textcolor{blue}{java.lang.NullPointerException}\\
		\tab at Ideone.closeReader(\textcolor{blue}{Ideone.java:26})\\
		\tab at Ideone.main(\textcolor{blue}{Ideone.java:8})
	\end{boxedminipage}
