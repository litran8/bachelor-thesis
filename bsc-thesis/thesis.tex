...
To better understand occurrences of \npes we present two different \ins{example} situations in which a \npe can be thrown.
...
By calling \code{createIconkit()} at line 7 the field \code{fManager} \chg{which}{(} is declared at line 3\ins{)} is \chg{initialized}{assigned } \textbf{null}. The field is assigned \textbf{null} by performing the method \code{getIconkitManager()} at line 14, which just returns the \textbf{null} value at line 19.
...
We are going to look only at the bytecode of method \code{say()} which is represented in between the lines 25-39 in \bytecoderef{helloWorld}. The green words in \bytecoderef{helloWorld} are \chg{called}{the string representations of } Java bytecode instructions. We will also use the term \textit{opcode} \ins{when} referring to individual instructions. Multiple opcode instructions are referred to as bytecode.

On the left-side of these instructions we see the \textit{program counter} (pc) which is a processor register that contains the address (location) of the instruction being executed at the current time. It is also called \ins{an} instruction pointer. We use these program counters, or pc-s also as an index in the bytecode. The instructions from pc 0 to pc 4 (lines 26-28) in \bytecoderef{helloWorld} represents the source code at line 5 in \coderef{helloWorld}. Therefore we use the term \textbf{pc-interval} to describe an instruction-set that represents \brs{represents what? you can't just give a few examples, say what those examples are}\eg a method invocation, an assignment \etc. This term will play an important role further in the thesis. We use the syntax \textbf{``<,>''} to represent the pc-interval. For the example just mentioned the pc-interval <0,4> represents the source code at line 5 in \coderef{helloWorld}. The first number indicates where the pc-interval starts and the latter one where it ends, both \chg{included}{inclusive}.

Going back to the example shown in \bytecoderef{helloWorld}, we can see on the right\del{-}side the name and type of variables (\eg line 27), or in case of an invocation the behavior/method name, the parameters it takes and the return type (\eg line 31). In \secref{lowLevelOverview} we will see how this information is used.

Underneath the instructions\ins{,} from line 33 \ins{un}till \del{the} line 36 in \bytecoderef{helloWorld} we find the \textit{line number table}\footnote{In the jargon of Javasist, this is also called the line number attribute.}. This line number table maps the pc to the source code line of the method \code{say()} represented in bytecode from line 25 to 39. \nm{From what I see, it actually indicates the starting pc for each line of code. Maybe we should mention that, just to make it clear.}\lt{in situations like a method call which is divided in multiple lines, it does not indicate the starting pc for each line of code..} Not only the method \code{say()} but each method bytecode (\eg the constructor \code{HelloWorld()} from line 9 to 21) has its own \textit{line number table}.
...
The bytecode of each behavior also holds a local variable table (\eg \bytecoderef{helloWorld} line 37-39) which is an array of local variables. All the parameters of the method \del{header} and the \del{values of the }local variables \del{which are created in the method body} are stored in this table. Parameters are stored first, beginning at index/slot 0. \del{But} the local variable table of a constructor or an instance method stores the \textit{``hidden this''} reference first, which is used to access the instance data of the object, then the parameters and so on\brs{this sentence is bad, rephrase pls}. In \figref{localVarArray}\footnote{Figure taken from \url{http://www.ibm.com/developerworks/library/it-haggar_bytecode/}} we see how a local variable is stored in the local variable table\brs{Not sure what the figure shows }.
...
As soon as the lifespan of a local variable ends, the slot can be reused by the next instantiated local variable. See the local variable table example shown in \bytecoderef{localVarTableDuplicatedSlot1} and the source code \coderef{localVarTableDuplicatedSlot1SourceCode} \ugh{which the local variable table belongs to}. Line 4 in \bytecoderef{localVarTableDuplicatedSlot1} holds the information about the local variable \code{url} at line 3 in \coderef{localVarTableDuplicatedSlot1SourceCode}. We see in the source code example \coderef{localVarTableDuplicatedSlot1SourceCode} that the variable \code{url} is only accessible within the \code{try\{\}} block which starts from pc 0 and ends with pc 40 \lt{not want to list the whole bytecode of the source code because it's too long...}. If we look further than pc 40 at bytecode-level, slot 2 is free again which is then immediately used by the next variable \code{e} in our example. This is why we can find local variable tables that contain multiple entries with the same local variable slot.
...
As explained in \charef{introduction}, this project is about providing the user with an additional link next to the common stack trace containing the \del{information/}location of \chg{where the null is assigned}{a \textit{null} assignment} to the variable that caused the \npe.
...
The general approach of NullSpy is to statically analyze a project \ins{in which \npe occurs} and instrument the \ins{corresponding} bytecode. NullSpy is a console application which takes two arguments. The first argument to NullSpy is the \ins{local} path to the folder containing the compiled class files of the original project and the second one is the \chg{destination where the}{path to the folder in which the} user wants to store the modified class files.

We provide the option to choose the destination in case the developer does not want to overwrite the original project with the modified one. This means that the updated bytecode can be stored in another location, keeping the original class files intact. Of course the original project can be replaced by the modified one\ins{,} too. But the developer has to think carefully about it because once replaced, the additional bytecode stays there and the execution time is slightly different than it was before\nm{ Add something like: (the explanation can be found in the \secref{execTimeDiff})}. To \chg{get rid of}{sweep away} the additional bytecode, the developer has to recompile the source code to get the original class files when needed.
...
\itemsep8pt
	...
	\item Modification: The modification part deals with the bytecode instrumentation and adding modules to the project that support\del{s} the inserted code.
...
Loading is done by traversing through all the subfolders of the folder\ins{ whose local path was} given as a parameter, and extracting all the \textit{.class} files. NullSpy modifies the class file directly when it is loaded. As soon as the modification is done, the class file is stored \chg{to}{at} the destination folder \ie \ins{the folder whose path is }the second parameter of NullSpy. \chg{Storing is done by recreating the structure from the source folder in the destination folder. \ugh{and storing the modified class files.  using the Javasist API.}}{The structure of the source original folder is preserved also at the destination folder.} \nm{I would not mention JavaAssist here, no need for it.}
...
In this section we will focus on the bytecode modification. \figref{modificationModules} shows that the modification part of NullSpy contains three different modules \del{in total}. \chg{That}{This} means\ins{ that} each class file of the loaded project will be run through \del{with} each of these modules. \chg{The first thing we do with the loaded class file is extracting}{After class loading, we first extract} data from \chg{its}{the class} bytecode, then \chg{do the instrumentation}{instrument it} and at the end we add the run\chg{ }{-}time \nm{Pay attention if it is ``run time", ``run-time" or ``runtime".\href{http://scg.unibe.ch/wiki/howtos/commonwritingerrors/Runtime}{Check out this link!}} supporter module to the project.
\nm{This paragraph is not exactly true. You do not add run-time support every time you load and instrument a class. You add it just once, at the end, right? That said, every class actually goes through two and three phases?}
...
From the stack trace of a \npe we can obtain three \ins{kind of }information\nm{information is uncountable, so there cannot be ``three information"}, namely the class name, method name and the line number of where the \npe happened. But the aim of NullSpy is to reveal the location of the null assignment \chg{of}{to} the variable which caused the \npe. To achieve this goal we need to take information from bytecode \nm{information about what?} and compare them \nm{What are ``them"? What do we compare with each other?} with each other to get the right location of the null assignment.

\nm{At the beginning of this paragraph, first explain what is the idea: why do you store anything about variables which are used as receivers for methods. They define the term ``method receiver", and precisely say what kind of information do you store, and why. When you define the term ``method receiver", say by which information is a method receiver uniquely identified: class name, method name etc. }
The first step in the modification part is to gather information about the \chg{\mr}{\mrs}. We use the term \textbf{\mr} to refer to variables on which methods are invoked, or whose fields are accessed. The same data as obtained from the \npe stack trace (class name, method name, line number) and other values \eg the variable name, variable type \etc are stored. \nm{where?}
...
The second step is to collect information about the null assignment\ins{s}. If a variable is assigned an object\nm{an object or null?}, we gather data about that variable from the bytecode. \nm{Here for example, you can actually leave the sentence like this ``we gather data" since you already explained in the previous paragraph exactly what kind of information do you store.} The most important information obtained is the \textit{line number} of the null assignment.\nm{Because of the previous sentence, I asked ``na object or null"?} Right after the assignment and the data collection \nm{You are here confusing two things: right after the assignment (you refer to place in the code?) and right after data collection (you here refer to the time - when?) }NullSpy inserts \chg{some}{the} bytecode that represents a method call which is managed by the \textit{run time supporter} package. This method takes those extracted information as parameters and checks whether the variable is assigned null. \nm{Ok, I suppose it was actually ``an object" at the beginning of the paragraph?} \nm{When does it check whether the variable is null? At static time or at run time?} \chg{If it is assigned null}{if this is the case,} the information about the variable is stored.

At the end of the bytecode modification part we wrap the \code{main()} method into a \textit{try-catch-block}. In the \textit{catch-block} those three information\ins{ mentioned before about the place in code where \npe happened} (class name, method name, line number) \del{mentioned before} are extracted from the \npe stack trace. \chg{These}{This} \nm{uncountable noun!} information \chg{are}{is} also \nm{why also? is something else also passed to this static method?} passed on to a static method of the run time supporter package. This method compares the parameters with the stored information about the \mrs. \nm{It feels like something is missing. NullSpy compares class name, method name and LOC of the place where \npe happened with method receivers? Don't you need also the information about the variables used in that line of code so that you can compare this with the stored data about method receivers?} If there is a match regarding the comparison, NullSpy creates a variable identification with the information about the matched \mr. With this id another comparison is performed. This time NullSpy compares the id with the information stored about the null assignment. The result will be the stored entry about the null assignment of the variable that caused the \npe. From the entry we extract the line number of the assignment and create the additional link that reveals the location of the null assignment. This link of course is then added to the common \npe stack trace.
...
We use the library Javassist to extract data from bytecode. Unfortunately, Javassist does not provide the functionality to directly get information \ins{we need} about the \mr\ins{, thus we need to do it manually}. \ins{This is why we implemented our own algorithm to get the \mrs.} The exact information we need is listed in \listref{methodReceiverInfo} and it is stored in an external \textit{csv} file due to overhead minimization.  \ins{We need the exact \textit{pc-interval} of the \mr to extract the right data from the bytecode.} \nm{What is right data? Listed in 3.4?}
...
\del{This is why we implemented our own algorithm to get the \mrs. We need the exact \textit{pc-interval} of the \mr to extract the right data from the bytecode.} As we discuss the steps of the algorithm we will be showing the result of each step when applied to the example source code and bytecode shown in \coderef{methodReceiverAlgDemoCode} and \bytecoderef{methodReceiverAlgDemoBytecode}. \nm{State want information you want to extract. At the end of the next paragraph, you say ``In our example, the end-pc is 155, since we are interested in the receiver of the \code{add} method." Say here: ``We are interested in the information blah, blah, blah...}The algorithm contains following steps:
...
Since the candidate pc-interval\del{s} end\ins{s} with the arguments of the method, and we know that the method expects $N$ arguments, we can ignore the last $N$ candidate pc-intervals. The one before them is the actual \mr pc-interval. In other words, the $N$-th to last candidate is the actual \mr pc-interval. In our example, since the method takes \chg{2}{two} arguments, the actual \mr pc-interval is <144,144>.
...
The \npe is treggered when the method \code{closeReader()} is called. There is an attempt at line 39 to close the reader, but because the field \code{reader} is assigned null the method call \code{close()} cause the \npe. Since only the \textit{IOException} is catched there the \npe goes up the call hierarchy until the \code{main()} method. As discussed in \subsecref{processFlow} in the catch-block of \code{main()} information is extracted from the \npe stack trace, multiple comparisons are performed and finally the link to the null assignment and the \npe stack trace are displayed.
